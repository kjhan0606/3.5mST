...
Figure~\ref{fig:mission-architecture-overview} summarizes the telescope and
its ultraviolet, visible, and infrared instrument suite. The baseline
parameters are listed in
Table~\ref{tab:baseline-telescope-specifications}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=1.0\linewidth,height=0.45\textheight,keepaspectratio]{book_media/image1.png}
...
Figures~\ref{fig:kasi-segment-phasing} and
\ref{fig:kasi-segment-integration} show the demonstrated KASI segment
phasing and integration hardware.
...
The conceptual instrument arrangement is shown in
Figure~\ref{fig:spectrograph-concept}.
...
Figure~\ref{fig:roman-coronagraph} shows the Nancy Grace Roman Space
Telescope coronagraph as a reference implementation.
...
The key observables describing cosmic acceleration are the distance--redshift relation and the cosmic expansion rate, H(z). These quantities can be measured directly through the Hubble diagram constructed using Type Ia supernovae as standard candles. By simultaneously utilizing a broad wavelength range from 0.2 to 1.5 μm, the telescope will enable supernova color corrections and evolutionary corrections to be performed within a single, internally consistent framework. This capability removes major limitations of ground-based observations, including variations in atmospheric transmission and wavelength-dependent calibration uncertainties, and enables stable photometric measurements with precision better than 0.01 mag.
...
The white-dwarf system considered in this section is illustrated in
Figure~\ref{fig:white-dwarf-concept}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth,height=0.42\textheight,keepaspectratio]{book_media/image6.jpeg}
  \caption{Conceptual illustration of a white dwarf. This is an
  AI-generated design study.}
...
The nucleosynthetic products associated with compact-star collisions are
summarized in Figure~\ref{fig:compact-object-nucleosynthesis}.
...
Figure~\ref{fig:hr8799-alpha-centauri-simulation} compares a demonstrated
direct-imaging result with the proposed mission concept.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth,height=0.42\textheight,keepaspectratio]{book_media/image8.png}
  \caption{Keck observations of HR 8799 in the left panel and an
  AI-simulated image of an exoplanet at 1\,au from Alpha Centauri A in the
  right panel. The simulation represents an expected observation with the
  3.5-meter segmented-mirror space telescope.}
  \label{fig:hr8799-alpha-centauri-simulation}
\end{figure}
...
In recent years, significant progress has been made in addressing observational biases and improving completeness corrections in exoplanet demographic studies. For example, statistical analyses of the Kepler catalog have incorporated corrections for variations in detection sensitivity and the effects of false positives, enabling the derivation of bias-corrected occurrence rates. These methods have been used to estimate fundamental population parameters, including the occurrence rate of Earth-like planets, commonly denoted as \textbf{η⊕} [69].
...
The classical Solar System habitable zone is illustrated in
Figure~\ref{fig:solar-system-habitable-zone}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth,height=0.42\textheight,keepaspectratio]{book_media/image9.png}
  \caption{Schematic diagram of the habitable zone in the Solar System. This
  is an AI-generated design study.}
  \label{fig:solar-system-habitable-zone}
\end{figure}
...
NEO surveillance using space-based telescopes offers significant advantages over ground-based observations. Ground observatories are constrained by atmospheric scattering, the day--night cycle, weather conditions, and seasonal visibility limitations, all of which directly affect observational efficiency. In contrast, space-based platforms operate free from these restrictions and can provide more continuous and comprehensive sky coverage. In particular, telescopes with high \textbf{near infrared (NIR) sensitivity} and wide-field survey capabilities are especially effective for detecting and tracking NEOs through their \textbf{thermal emission}, enabling the discovery of objects that may be difficult to detect in reflected visible light [13].

Figures~\ref{fig:known-neo-distribution} and
\ref{fig:owl-observatory} show the current NEO census context and the KASI
ground-based surveillance capability.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth,height=0.42\textheight,keepaspectratio]{book_media/image10.png}
  \caption{Visualization of the distribution of more than 37,000 known
  near-Earth objects using NASA's Eyes on Asteroids. This is an AI-generated
  design study.}
  \label{fig:known-neo-distribution}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=1.00\linewidth,height=0.5\textheight,keepaspectratio]{book_media/image11.jpeg}
  \caption{KASI's Optical Wide-field Patrol system for space-object
  surveillance in the left panel and the OWL observatory in Mongolia in the
  right panel.}
  \label{fig:owl-observatory}
\end{figure}
...
Observations conducted with a space telescope can simultaneously provide measurements of NEO brightness and motion across multiple wavelength bands. In particular, \textbf{near infrared observations} offer a major advantage because they can detect both reflected sunlight and the object\textquotesingle s own \textbf{thermal emission}. By combining these measurements, astronomers can determine an object\textquotesingle s size and albedo with substantially greater accuracy than is typically achievable using visible-light observations alone. As a result, space-based near infrared surveys provide a more reliable characterization of NEO populations and significantly improve our ability to assess potential impact hazards [44].
...
In particular, \textbf{near infrared spectroscopic observations} play a crucial role in accurately characterizing NEOs. Observations based solely on reflected sunlight suffer from a degeneracy between an object\textquotesingle s size and albedo: two objects with the same apparent brightness may differ significantly in their actual sizes and reflectivities. In contrast, near infrared thermal-emission measurements depend directly on an object\textquotesingle s surface temperature and physical size. By combining reflected-light and thermal-emission observations, astronomers can disentangle these effects and derive more reliable estimates of size, albedo, density, and surface properties.
...
To overcome this limitation, observations of an NEO's \textbf{thermal emission} are required. Near infrared measurements provide direct information about an object\textquotesingle s thermal radiation, allowing estimates of its surface temperature and physical size. When combined with reflected-light observations, thermal-emission data help break the degeneracy between size and albedo, enabling more accurate determination of both parameters [13]. This combined optical--near infrared approach allows the estimation of absolute sizes and physical classifications that cannot be achieved through optical observations alone, thereby reducing uncertainties in impact-consequence models.
...
A space telescope equipped with sensitive near infrared instrumentation can increase the detection rate of small NEOs that are difficult to discover using ground-based surveys alone. In particular, objects with diameters below approximately \textbf{30 m} fall within a regime where ground-based sensitivity is often limited by atmospheric effects and background noise. Space-based observations can substantially improve detection completeness in this size range by identifying objects through their thermal emission.
...
By combining optical and near infrared spectroscopic capabilities, a space telescope can simultaneously measure an object\textquotesingle s size, albedo, and spectral characteristics. These measurements substantially narrow the range of input parameters used in impact-consequence calculations and hazard-assessment models. In particular, high-resolution spectroscopy can provide valuable compositional information, enabling more accurate estimates of material properties and improving models of impact-energy deposition and fragmentation behavior during atmospheric entry or surface impact.
...
By combining optical and near infrared observations, a space telescope can simultaneously determine an NEO's size, albedo, and spectral composition. These parameters constitute critical inputs for impact-consequence models. In particular, the combined analysis of reflected and thermal radiation breaks the degeneracy between size and albedo, significantly reducing uncertainties in estimates of impact energy and potential damage [13].
...
The left panel of Figure~\ref{fig:annual-field-of-regard} does not explicitly
represent the orbit--Sun geometry of a Sun-synchronous orbit (SSO). It is
more accurately interpreted as a map of the annual observable days for a
space telescope operating with a solar avoidance angle of $60^\circ$,
regardless of its specific orbit. The field of regard was calculated by
counting the days when the angular separation between a target and the Sun
is at least $60^\circ$. The theoretical visibility ranges from approximately
243 days per year near the ecliptic plane to 365 days per year near the
ecliptic poles. The right panel gives the corresponding result for a
Sun--Earth L2 observatory and yields essentially the same distribution.


\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth,height=0.42\textheight,keepaspectratio]{book_media/image12.png}
  \caption{Annual field of regard for the 3.5-meter space telescope under a
  $60^\circ$ solar avoidance constraint. The left panel shows the 700\,km
  Sun-synchronous-orbit case and the right panel shows the Sun--Earth L2
  case. The color scale gives the annual observable time from 243 days near
  the ecliptic plane to 365 days near the ecliptic poles. This is an
  AI-generated design study.}
  \label{fig:annual-field-of-regard}
\end{figure}
