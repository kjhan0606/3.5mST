...
A $1.3\,\mu$m cutoff cannot execute the stated BAO and RSD survey. The cutoff removes H$\alpha$ before the survey reaches $z=1$ and removes [O\,III] above $z=1.60$. A $2.0\,\mu$m cutoff is a substantial improvement, but it is not equivalent to continuous rest-optical coverage through $z=3$. Above $z\simeq2.05$, the catalog changes from a mixed Balmer and oxygen-line selection to an oxygen-line selection. This discontinuity changes galaxy bias, redshift success, and completeness because H$\alpha$ is a recombination line tied to the ionizing massive-star population, while [O\,III] and [O\,II] respond strongly to metallicity and ionization state \cite{kennicutt1998,khostovan2015,saito2020}. The resulting selection change occurs in the same redshift range where the mission aims to interpret the anisotropic clustering amplitude as growth rather than as an evolving tracer systematic.

The physically traceable threshold is therefore $2.70\,\mu$m. At $z=3$, H$\alpha$, [N\,II] $6584$\,\AA, and [S\,II] $6731$\,\AA\ appear at $2.625$, $2.634$, and $2.692\,\mu$m. Keeping the complete complex in band provides a Balmer-selected control population, confirms oxygen-line redshifts, and measures line ratios that diagnose active nuclei and changing nebular conditions. Retaining these galaxies also protects the effective number density $n(z)$ and hence the effective clustering volume defined later in Equation~\eqref{eq:veff}. A loss of one tracer is amplified when $nP$ approaches unity because the statistical weight scales as $[nP/(1+nP)]^2$.

\begin{leadbox}[title=Cosmology Wavelength Requirement]{green}
The near-infrared channel shall provide calibrated science throughput through $2.70\,\mu$m. The adopted operational band edge is $3.0\,\mu$m. The guard band keeps the $z=3$ H$\alpha$+[N\,II]+[S\,II] complex inside the high-throughput region after filter transitions, detector quantum-efficiency roll-off, wavelength-calibration edge losses, and flight margin are included. The $3.0\,\mu$m value also extends H$\alpha$ to $z=3.57$ and [O\,III] to $z=4.99$. The extended redshift reach is added capability rather than the basis of the threshold. Phase A shall compare $1.30$, $2.00$, $2.70$, and $3.00\,\mu$m cutoffs with the same luminosity functions, ETC, deblending model, and survey time. The decision data products are $n(z)$, secure-redshift fraction, line-confusion rate, $nP$, $V_{\rm eff}$, and forecast uncertainties in $D_M/r_d$, $Hr_d$, and $f\sigma_8$.
\end{leadbox}
