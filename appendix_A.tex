...
\textbf{The \facility\ uses an image slicer for all spectroscopic observations. The instrument study compares $R=1000$ and $R=5000$ before selecting the final resolving power.}
...
The image slicer forms part of the adopted spectrograph architecture. The slicer reformats a wide entrance region into a pseudo-slit without imposing the light loss of a narrow physical slit. The instrument team has not selected the resolving power. The cosmology study must therefore compare $R=1000$ and $R=5000$ within the same image-slicer architecture. The comparison depends on redshift success, catastrophic line misidentification, spectral overlap, detector format, survey area, $nP$, and the resulting BAO and RSD precision. The numerical yield estimates below use $R=1000$ as a reference calculation rather than an adopted instrument specification.
...
Spectroscopy & Image-sliced grism or prism channels. The resolving-power trade compares $R_{\rm point}=1000$ and $R_{\rm point}=5000$ within the same architecture.\\
...
Field of view & \fovsmall\ threshold and \fovlarge\ goal. Phase A must determine how detector format and spectral length change the recorded field at each candidate resolving power.\\
...
The spectrograph is an image-sliced two-arm system. A dichroic beam splitter sends 0.3--1.0\,$\mu$m light to the optical arm and 1.0--3.0\,$\mu$m light to the near-IR arm. Each arm uses band-limiting filters and a grism or prism disperser so that a crowded field does not produce decade-long spectra. It is desirable to observe with both arms simultaneously. Otherwise a revisit to the tile should use the same orientation and pointing. The near-infrared arm must retain science throughput to at least $2.70\,\mu$m and adopts $3.0\,\mu$m as the operational band-edge goal. The additional $0.30\,\mu$m provides a throughput guard band. The guard band keeps the $z=3$ H$\alpha$+[N\,II]+[S\,II] complex away from filter transitions, detector quantum-efficiency roll-off, and wavelength-calibration edge losses. The optics should be cooled until telescope thermal self-emission remains below the zodiacal background at the reddest wavelengths. The exposure-time calculation of Section~\ref{sec:etc} sets the requirement. The optics reach the background-limited floor across the full $1$--$3\,\mu$m band at an operating temperature $\lesssim200$\,K.
...
The image slicer acts as a wide entrance slit. The slicer accepts the same source area at both resolving powers and assigns each spatial element to a spectral trace. Changing $R$ redistributes the same source and sky photons among a different number of spectral samples. The S/N integrated over the same physical emission line or the same final wavelength interval is therefore independent of $R$ in the photon-limited limit. Binning an $R=5000$ spectrum to the wavelength bins used at $R=1000$ recovers the same S/N. The native S/N in one unbinned $R=5000$ sample is lower because the sample spans a narrower wavelength interval. The lower value does not represent a loss in the cosmological redshift measurement because the redshift fit combines the samples across the line profile. Read noise, dark current, scattered light, spectral cross-talk, and extraction losses introduce a residual dependence when the higher resolving power illuminates more pixels.
...
\textbf{Choose $R=5000$ if both designs preserve the same field, wavelength coverage, throughput, and observing time.}
The $R=5000$ spectrum retains the integrated photon-limited S/N after binning and adds information that separates the [O\,\textsc{ii}] doublet and rejects incorrect line identities. The $R=1000$ design becomes preferable only when the longer $R=5000$ traces reduce usable sky area or wavelength coverage enough to erase the gain in redshift purity. The deciding quantity is the final BAO and RSD covariance from the recovered catalog.
\end{leadbox}
...
\begin{tikzpicture}[font=\sffamily\footnotesize,scale=0.98, every node/.style={transform shape}]
  ...
  {Each detector sees reformatted field slices. Filter choice limits trace length and three orientations change the remaining blend pattern.};
\end{tikzpicture}
...
\caption{Unsliced control simulation of crowded-field slitless exposures with $0.11''$ pixels and the $1.0$--$1.75\,\mu$m band-limited setting at $R=1000$. The constant dispersion gives $1091$-pixel first-order traces. Sources follow deep near-infrared counts, $40$ per arcmin$^2$ over $18.5<m_{\rm AB}<24.5$, with FSPS galaxy template spectra at $0.4<z<2.2$. \emph{Top left} shows the direct image of the displayed $88''\times88''$ field. The other panels show the same field dispersed at three survey roll angles. Traces from sources outside the crop enter the displayed region as on a real detector. Every source becomes a long one-sided trace and emission lines appear as compact knots. Neighbouring spectra overlap heavily at one orientation. Rotating the dispersion by $45^\circ$ and $90^\circ$ changes which pairs overlap. The control case demonstrates the confusion that the adopted image slicer and the joint scene reconstruction of Eq.~\eqref{eq:scene} must reduce. The panel illustrates overlap geometry rather than completed recovery statistics. Section~\ref{sec:e2eval} defines the required survey-scale tests of purity, completeness, and flux bias.}
\label{fig:crowded}
\end{figure}

The three-roll strategy changes the overlap geometry between exposures. The adopted image slicer cuts the focal-plane region into strips and reformats the strips into a pseudo-slit. The same geometry applies to both candidate resolving powers. Sources in different slices do not overlap. Sources that share a slice can still blend in wavelength. The restricted adjacency reduces the number of coupled sources relative to the unsliced control in Figure~\ref{fig:crowded}. Image slicers operate in flight instruments including the JWST NIRSpec integral-field unit \cite{boker2022}. The flight heritage establishes the optical principle. The proposed wide-field implementation still requires an optical demonstration.
...
The sensitivity of the image-sliced two-arm spectrograph and the survey planning depth adopted in Section~\ref{sec:elgdepth} follow from a detector-level photon budget. The calculation must separate the absolute sensitivity estimate from the comparison of resolving power. For an emission line of flux $F_{\rm line}$ observed at wavelength $\lambda$, the number of detected photo-electrons accumulated over a total exposure $t$ is formulated as
\begin{equation}
...
\end{equation}
The factors $\delta\lambda_{\rm pix}$ and $N_\lambda$ cancel. Equation~\eqref{eq:etcslicerbg} therefore contains no explicit resolving power. Adding source shot noise, the integrated sky, dark current $D$, and read noise $\sigma_{\rm read}$ over $n_{\rm det}$ illuminated detector samples gives the total variance
\begin{equation}
  N^2=S+B_{\rm sci}t+D\,n_{\rm det}t+n_{\rm read}\,n_{\rm det}\,\sigma_{\rm read}^2,
  \label{eq:etcnoise}
\end{equation}
and the $5\sigma$ line-flux limit is the flux for which $S=5N$. For the space near-infrared background the sky term dominates when the detector terms remain small. Setting $S=5N$ with $F_{\rm line}=F_{5\sigma}$ gives
\[
...
Roman's High Latitude Spectroscopic Survey provides the closest design precedent. It is a large-area slitless survey with $R\simeq435$--865 that forecasts millions of H$\alpha$ and [O\,III] redshifts and explicitly targets BAO and RSD cosmology \cite{wang2021}. The $R=1000$ candidate retains line-separation margin while keeping spectra shorter. The $R=5000$ candidate resolves more line structure and reduces ambiguous identifications. The goal is a calibrated high-purity redshift sample whose radial smearing is small compared with the BAO scale after quality cuts. Internal kinematics provide an additional return rather than the primary cosmology criterion.
...
The method combines two mission data products. Diffraction-limited imaging constrains the Einstein radius $\theta_E$, the projected mass inside the radius, and the radial slope $\gamma'$ from the reconstructed ring. An image-sliced spectrum adds an aperture-integrated estimate of the deflector stellar velocity dispersion $\sigma$. The $R=1000$ candidate has a point-source width near $300$\,\kms\ and limits low-dispersion systems. Lens-selected deflectors are commonly massive ellipticals with $\sigma\gtrsim250$\,\kms. Template fitting and morphology deconvolution across three roll angles may recover an integrated dispersion for population analyses. The $R=5000$ candidate reduces the point-source width to about $60$\,\kms\ and provides the stronger measurement for a selected lens sample without a photon-limited S/N penalty after integration over the same absorption feature. The slice geometry still does not guarantee spatially resolved kinematics. Flagship systems that require two-dimensional stellar-velocity fields need external integral-field spectroscopy \cite{tdcosmo2025}.
...
Pathfinders and contingency & Years 3--5 & 300--700 hr & Resolving-power calibration with image-sliced ELGs, strong-lens targets, LBG spectroscopy, ICL cluster imaging, and special-field programs.\\
\bottomrule
\end{tabularx}
...
The image slicer defines the spectroscopic architecture at both candidate resolving powers. A direct image measures target positions and the orientation of extended sources. The slicer then reformats the field into a pseudo-slit and the disperser records one or more band-limited settings. Compact ELGs provide the primary calibration sample because their [O\,\textsc{ii}], H$\beta$, [O\,\textsc{iii}], and H$\alpha$ profiles permit direct comparison with external spectroscopy. The observing sequence records at least two dithers across slice boundaries and repeats an orientation when source morphology would otherwise dominate the line-spread function.

The science team should recommend $R=1000$ or $R=5000$ only after four matched tests. The optical models must fit the same wavelength range and report the usable field on the adopted detector format. Calibration exposures must constrain wavelength and line-spread-function changes across slices. Crowded-scene injections must recover line flux and redshift without significant bias. The image-sliced ETC must reproduce equal photon-limited S/N over the same physical line interval and quantify detector-dependent departures. The final forecast must propagate the recovered completeness, catastrophic-redshift fraction, usable area, and $n(z)$ into BAO and RSD covariance. The present $R=1000$ yield remains a reference value until the matched test selects the flight specification.
