...
\textbf{The \facility\ combines a full-field $R=1000$ slitless survey mode with deep imaging and a restricted-field $R=5000$ image-sliced mode for selected targets.}
The central science case is a dense emission-line galaxy redshift survey for BAO and redshift-space distortions. Supernova and quasar programs exploit the stability, multiplexing, and repeatability of space operations. The supernova tier measures the rest-frame $U$ and near-ultraviolet magnitudes that separate optical twins at subgroup precision ($\sigma_m=0.10$\,mag) to $z\simeq0.9$--$1.1$ in standard visits and to $z\simeq1.3$--$1.5$ in ten-hour stacks. Every wide-survey tile receives three slitless orientations near $0^\circ$, $45^\circ$, and $90^\circ$. A joint scene reconstruction uses the changing overlap geometry to recover the spectra. The flagship survey covers 100--300 \degti\ and requires the \fovlarge\ field for efficiency. A deep pencil-beam tier and a supernova time-domain tier complement the wide survey. The same imaging provides a blind census of ultra-diffuse galaxies, low-surface-brightness galaxies, and void dark-galaxy candidates across the survey tiers. Targeted deep integrations map intracluster light in nearby clusters. Emission-line redshifts recover part of the low-surface-brightness population missing from the observed galaxy stellar mass function and turn its faint end into a selection-controlled test of stellar feedback. The common selection function across field, cluster, and void environments supports a dark-matter program that tests cold, self-interacting, and fuzzy dark matter against dwarf-galaxy inner density structure and the abundance of low-mass halos.
\end{leadbox}
...
\begin{tikzpicture}[font=\sffamily]
  \foreach \x/\title/\val/\col/\vfont in {
    0/{Survey / sliced modes}/{$R=1000/5000$}/teal/\large,
...
The full-field $R=1000$ slitless mode defines the flagship survey because the leading cosmological measurements depend on survey volume, line identification, source confusion, and selection-function calibration. The repeatedly calibrated map of emission-line galaxies provides more cosmological information than precision internal kinematics for a small target sample. The restricted-field $R=5000$ mode serves a different purpose. An image slicer reformats narrow regions of the focal plane into a pseudo-slit and measures selected compact sources at higher spectral resolution. The high-resolution mode does not inherit the \fovlarge\ survey field or the wide-tier cadence.
...
\end{equation}
The corresponding point-source widths are about $300$\,\kms\ at $R=1000$ and $60$\,\kms\ at $R=5000$. The survey mode measures redshifts for a statistical clustering analysis rather than peculiar velocities of individual galaxies. RSD constrains $\fsig(z)$ through the anisotropic galaxy power spectrum $P(k,\mu)$ or correlation function. Redshift success, purity, survey-window control, and sufficient $nP$ set the wide-tier requirements. The sliced-field mode instead targets resolved line profiles, the [O\,\textsc{ii}] doublet, and stellar absorption features in selected systems.
...
\end{equation}
or about 4600 pixels at two pixels per resolution element. The whole-wavelength dispersion is still too long to lay over a crowded field. From the start, the baseline design is therefore to handle it with two band-limited arms and three orientations per tile.
...
Spectroscopy & Full-field slitless grism or prism channels with $R_{\rm point}=1000$. A separate image-sliced configuration provides $R_{\rm point}=5000$ over a restricted field.\\
Wavelength strategy & 0.3--1.0\,$\mu$m optical channel and a near-IR channel with science throughput to at least 2.70\,$\mu$m. The adopted detector and filter band-edge goal is 3.0\,$\mu$m. Each channel is divided into filters to limit spectrum length.\\
Field of view & \fovsmall\ threshold and \fovlarge\ survey modes at $R=1000$. Phase A must set the smaller image-sliced field from the detector format and slice geometry.\\
Roll strategy & Three $R=1000$ slitless orientations per science tile near $0^\circ$, $45^\circ$, and $90^\circ$. The $R=5000$ target sequence uses the number of orientations required by the source geometry.\\
...
The spectrograph is a slitless two-arm system. A dichroic beam splitter sends 0.3--1.0\,$\mu$m light to the optical arm and 1.0--3.0\,$\mu$m light to the near-IR arm. Each arm uses band-limiting filters and a grism/prism disperser so that a crowded field does not produce decade-long spectra. It is desirable to observe with both arms simultaneously. Otherwise a revisit to the tile should be scheduled with the same orientation and pointing. The near-infrared arm must retain science throughput to at least $2.70\,\mu$m and adopts $3.0\,\mu$m as the operational band-edge goal. The additional $0.30\,\mu$m is a throughput guard band rather than an arbitrary extension of the survey. The guard band keeps the $z=3$ H$\alpha$+[N\,II]+[S\,II] complex away from filter transitions, detector quantum-efficiency roll-off, and wavelength-calibration edge losses. The optics should therefore be cooled so that telescope thermal self-emission stays below the zodiacal background at the reddest wavelengths. The exposure-time calculation of Section~\ref{sec:etc} sets this requirement, with the optics reaching the background-limited floor across the full $1$--$3\,\mu$m band for an operating temperature $\lesssim200$\,K.

The filter plan follows two properties of grism dispersion. A grism disperses at nearly constant $\Delta\lambda$ per pixel rather than constant $R=\lambda/\Delta\lambda$. The resolving power rises approximately linearly across one setting. The quoted $R=1000$ and $R=5000$ values refer to the centre of the corresponding setting. A grating also produces overlapping orders because second-order light at $\lambda$ lands at the same detector position as first-order light at $2\lambda$. At most one octave, $\lambda_{\max}\leq2\lambda_{\min}$, therefore remains free of cross-order contamination. The survey observes each arm through two band-limiting filters that obey the octave limit. The optical arm covers $0.36$--$0.70$ and $0.50$--$1.0\,\mu$m. The near-infrared arm covers $1.0$--$1.75$ and $1.6$--$3.0\,\mu$m. HST, JWST, Euclid, and Roman slitless instruments use the same order-sorting principle. The two settings per channel define the exposure times and the survey schedule in the proposal. The $R=1000$ survey traces occupy about $1090$--$1330$ pixels at two pixels per resolution element. A full-field $R=5000$ spectrum would require about five times the trace length. The detector format therefore couples the $R=5000$ option to an image slicer and a restricted field.

The two spectral modes have different background terms. A full-field slitless detector pixel receives diffuse sky over the band-limiting filter because every sky position contributes a dispersed spectrum. Increasing the slitless resolving power by a factor of five lengthens every source trace by the same factor while leaving the sky rate per detector pixel nearly unchanged. Binning a full-field $R=5000$ exposure back to $R=1000$ then sums five sky-dominated pixels. The resulting S/N is lower by $5^{1/2}\simeq2.2$ and recovering the original S/N requires about five times the exposure. The calculator of Section~\ref{sec:etc} gives the same result. A full-field dispersion test reduces the binned synthetic-band S/N of a peak $z=1$ supernova from $12.2$ to $5.4$ in a two-hour exposure.

An image slicer changes the mapping. Each detector pixel receives one spatial element over one spectral interval rather than diffuse light integrated over the full filter. Binning an ideal $R=5000$ sliced spectrum to $R=1000$ therefore recovers the photon-limited continuum S/N apart from slicer throughput, detector noise, scattered light, and cross-talk. Native-resolution continuum S/N per resolution element still scales as $R^{-1/2}$ because the photons occupy a narrower wavelength interval. An unresolved emission line follows the opposite trend. Its total line flux remains fixed while the sky window narrows, which improves the line S/N as $R^{1/2}$ until the instrumental width reaches the intrinsic line width. The image slicer also limits the source width projected into the dispersion direction and suppresses overlap between different slices.

The gain does not preserve the survey grasp. The pseudo-slit length, detector area, and allowed trace length set a restricted instantaneous field at $R=5000$. Slice losses and extra optical surfaces also lower the throughput. Phase A must measure the slice width, field, throughput, point-spread function, spectral cross-talk, and calibration stability before the $R=5000$ mode enters a yield forecast. The $R=1000$ mode remains the only basis of the flagship ELG count and survey-time estimates. The $R=5000$ option supports selected compact emission-line galaxies, [O\,\textsc{ii}] doublet measurements, and strong-lens deflectors. Broad supernova features and reionization damping wings retain the $R=1000$ mode. Individual Ly$\alpha$ forest absorbers still require $R\gtrsim30{,}000$ and remain outside both modes.
...
Slitless spectroscopy in crowded extragalactic fields is a deliberate design choice of the wide survey, not a limitation to work around. In dense extragalactic fields a single dispersion direction is not sufficient because the dispersed spectra of neighbouring galaxies overlap in a pattern that depends on wavelength, object morphology, and roll angle. The baseline extraction therefore uses a \textbf{multi-orientation scene-reconstruction algorithm} rather than independent one-dimensional extractions.
...
\begin{equation}
  ...
  \label{eq:scene}
\end{equation} 
where $\mathbf{d}_{r}$ is the detector image at roll/orientation $r$, $\mathbf{s}$ is the set of object spectra, and $\mathbf{A}_{r}$ is the orientation-dependent projection operator built from the direct-image catalog, morphology model, trace solution, PSF, and wavelength calibration. The spectra are recovered by a regularized least-squares inversion of this over-determined system, solved with sparse iterative methods rather than an explicit matrix inverse. Smoothness, non-negativity, and machine-learning priors, together with the direct-image morphology, stabilize the fully blended directions, and the solution covariance and contamination flags are propagated into the redshift catalog. Objects that remain degenerate after the three-angle fit are not forced into the cosmology sample. They are flagged as low-confidence redshifts or excluded by the survey mask.

The most cost-effective wide-survey choice is triple-roll orientations. Two nearly orthogonal orientations are the minimum useful configuration, but they leave a non-negligible class of pair geometries, detector defects, and extended-source self-blends with ambiguous solutions. A third roll separated by roughly $45^\circ$ from the orthogonal pair changes the blending pattern without multiplying the program into a deep-field campaign. Additional rolls are scientifically useful only for reference fields, ultra-deep calibration fields, or unusually crowded targets because their survey-area cost is linear while the deblending gain becomes incremental.
...
The baseline deblending pipeline uses the most established approach for precision slitless work. This approach combines calibrated forward modeling, simultaneous fitting of all cataloged sources, and regularized sparse linear inversion across all rolls and dithers. This follows the operational logic of HST slitless extraction software, where direct-image positions, morphology, trace solutions, flat-field cubes, and contamination models define the extraction geometry \cite{axe2009}. It also follows the LINEAR formalism, where all transformations between direct-image sources and dispersed pixels are assembled into a large sparse linear system solved by LSQR \cite{linear2018}. The same principle is used in crowded-field 3D spectroscopy. Spectra are extracted by fitting many sources simultaneously with a wavelength-dependent PSF and an external high-resolution catalog prior \cite{kamann2013,husser2016}. Machine learning is therefore not the primary deblender. It plays a constrained, supporting role for catalog quality assurance, missing-neighbor discovery, trace-mask generation, artifact rejection, and flagging of high-contamination pixels. Convolutional or instance-segmentation networks can flag these inputs following astronomical Mask R-CNN deblending work \cite{burke2019}. Bayesian variational inference provides posterior uncertainties for source counts, positions, and fluxes \cite{liu2021}. Those posteriors inform subsequent estimates of contamination and redshift quality. Non-negative matrix factorization has reduced calibration crosstalk in the Keck OSIRIS integral-field spectrograph and provides a relevant test for calibration components here \cite{horstman2022}. Generative models, including diffusion, are not part of the baseline cosmology extraction unless they are constrained to reproduce the detector pixels through the calibrated forward model and pass injection-recovery bias tests. The success criterion is therefore not visual cleanliness. It is recovered line flux bias, redshift failure rate, contamination covariance, and injection-recovery completeness as functions of neighbor separation, magnitude contrast, morphology, and roll geometry.

The machine-learning literature on deblending spans several families. It is useful to place each relative to this calibrated forward model. The first is constrained matrix factorization, exemplified by SCARLET, which models every source as a small number of components, each the product of a spectral energy distribution and a non-parametric spatial profile. The method imposes physical constraints such as non-negativity, monotonicity from the peak, and symmetry to make the separation well posed \cite{scarlet2018}. It is a semi-parametric optimization method rather than a trained network. It is the production deblender of the Rubin Observatory Science Pipelines. The second family is deep generative priors. Variational autoencoders have been applied directly to astronomical deblending \cite{arcelin2021}. Deep generative galaxy models provide morphology priors and realistic simulation populations \cite{lanusse2021}. Normalizing flows and score-based or diffusion models belong to the same broader family. The cited studies do not establish line-flux bias control for slitless spectra. The third family is instance-segmentation networks such as Mask R-CNN that detect and mask overlapping sources at the pixel level \cite{burke2019}. The fourth is probabilistic and variational catalog inference that returns full posteriors on source positions and fluxes \cite{liu2021}. For a slitless cosmology survey the decisive property is not raw image-plane separation quality but control of line-flux bias. Therefore, the present baseline uses these methods as priors and quality checks inside the forward model rather than as standalone image cleaners. Any learned prior should reproduce the detector pixels through the same calibrated operator and pass injection-recovery tests before it enters the cosmology path.

The two wide-field space missions closest to this concept have made deliberately different choices. Both inform the baseline. The Euclid Morphology Challenge compared SourceXtractor++ with four other profile-fitting codes on about 1.5 million simulated galaxies. The tests forecast robust structural measurements for at least 400 million survey galaxies and also show the loss of accuracy caused by non-analytic morphologies \cite{sourcext2020,euclidmorph2023}. Euclid's SIR spectroscopic processing function uses a photometric source catalog to extract NISP spectra and model cross-contamination \cite{euclidsir2026}. Roman extracts its grism spectra with pyLINEAR, the sparse linear inversion that assembles every transformation between direct-image sources and dispersed pixels into one large system \cite{linear2018,wang2021}. Roman supernova simulations have demonstrated the multi-roll extension. The method reconstructs a three-dimensional scene from slitless spectra at the available roll angles and separates host-galaxy light from the supernova \cite{astraatmadja2026}. Forward-model emulators such as ESpRESSO now generate realistic Roman slitless scenes for training and validation \cite{espresso2024}. Rubin, whose data are imaging rather than slitless, deblends with the constrained SCARLET factorization \cite{scarlet2018}. The present proposal sits closest to the Roman lineage because its science is slitless and its defining asset is multi-orientation coverage. It adopts the Roman and HST forward model of Eq.~\eqref{eq:scene} as the baseline, using the Euclid model-fitting priors and the Rubin and generative machine-learning methods as constrained support.
...
The three-roll strategy preserves the full survey field and changes the overlap geometry between exposures. An image slicer provides the second spectroscopic mode. The slicer cuts a selected focal-plane region into narrow strips, reformats the strips into one pseudo-slit, and disperses the pseudo-slit at $R=5000$. Sources in different slices do not overlap. Sources that share a slice can still blend in wavelength. The restricted adjacency reduces the number of coupled sources relative to the full-field scene in Figure~\ref{fig:crowded}. Image slicers operate in flight instruments including the JWST NIRSpec integral-field unit \cite{boker2022}. The flight heritage establishes the optical principle but does not establish a wide-field implementation for the proposed telescope.

The sliced mode is limited by detector format and survey grasp. A fivefold increase in resolving power requires about five times as many spectral samples over the same band. Reformatting the full \fovlarge\ field would exceed a practical pseudo-slit length and would multiply the detector area, optical complexity, calibration load, and exposure time. The proposal therefore assigns $R=5000$ to selected regions in the medium, deep, and ultra-deep tiers and to targeted compact sources. The $R=1000$ three-roll mode remains the wide-tier baseline. Phase A must determine the slice width and field from an end-to-end optical model. The same study must demonstrate throughput, cross-talk, wavelength stability, and blind recovery of injected sources before the high-resolution mode receives committed survey time.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{hsc_slicer_demo.png}
\caption{Image-slicer geometry applied to a scene whose source positions, brightnesses, sizes, ellipticities, and position angles come from a Subaru HSC-SSP deep image of the SXDS/UDS/XMM-LSS field \cite{aihara2018}. The rendering uses the proposed $0.11''$ pixels and diffraction-limited point-spread function. Each row shows one orientation. The left panel divides the reconstructed image into five horizontal slices. The right panel reformats the slices into one pseudo-slit and disperses the result. The demonstration uses $R=1000$ to keep its image scale directly comparable with Figure~\ref{fig:crowded}. The proposed flight option would replace the disperser with the $R=5000$ setting and would accept a smaller field set by the detector format. Dashed lines mark the segment assigned to each slice. Sources in different slices do not overlap. The five-slice layout illustrates the mapping and does not define the final optical design.}
\label{fig:slicer}
\end{figure}
...
\begin{equation}
  ...
  \label{eq:reff1000}
\end{equation}
At 1\,$\mu$m, a 3.5 m diffraction-limited telescope has a point-spread-function FWHM near $0.06''$. With $R_{\rm point}=1000$, Eq.~\eqref{eq:reff1000} gives $R_{\rm eff}\simeq510$ for a $0.10''$ emitting region and $R_{\rm eff}\simeq290$ for a $0.20''$ emitting region. The values meet the ELG redshift requirement but do not support precision internal kinematics. An ideal $R_{\rm point}=5000$ slitless trace would raise the corresponding values to about $2550$ and $1450$. An image slicer provides an additional advantage because the slice width limits $\theta_{\rm src,\parallel}$. The actual $R_{\rm eff}$ then depends on slice width, source centering, optical aberrations, and the line-emitting morphology. Phase A must measure the recovered line-spread function with extended-source scenes rather than infer the performance from point sources alone.

Extended galaxies also create a second, internal deblending problem. Because different parts of the same galaxy project to different detector pixels along the dispersion direction, spatial structure mixes with wavelength. This morphology-induced self-blending is handled inside the same calibrated forward model, not by a separate image-cleaning step. The direct image supplies a wavelength-zero-point prior, segmentation map, Sersic or multi-component morphology, and clump masks. For galaxies whose emission-line morphology is not well represented by the broadband continuum, the pipeline fits a small number of internal components,
\begin{equation}
  \mathbf{d}_{r,g} =
  \sum_{c=1}^{N_c}
...
The field of view changes the mission from a deep-field spectroscopic facility into a cosmological surveyor. The two allowed field sizes differ by a factor of nine in area as
\begin{align}
  A(10'\times10') &= 100\ {\rm arcmin^2}=0.0278\ {\rm deg^2},\\
  A(30'\times30') &= 900\ {\rm arcmin^2}=0.2500\ {\rm deg^2}.
\end{align}
For a 100 \degti\ survey, the threshold field requires roughly 3,600 pointings before the required three orientations and dithers while the maximum field requires 400. For 300 \degti, those numbers become 10,800 and 1,200. Including the three slitless orientations, the raw spectroscopic visit count is multiplied by three. The \fovsmall\ mode is useful for deep calibration fields and time-domain fields. The \fovlarge\ mode should be treated as the cosmology survey requirement.
...
The sensitivity of the two-arm slitless spectrograph and the corresponding survey planning depth adopted in Section~\ref{sec:elgdepth} (Eq.~\eqref{eq:f5sigma}) follow from a detector-level photon budget. It is worth deriving that depth from first principles and confirming both its value and its background-limited time scaling. For an emission line of flux $F_{\rm line}$ observed at wavelength $\lambda$, the number of detected photo-electrons accumulated over a total exposure $t$ is formulated as,
\begin{equation}
  S=F_{\rm line}\,A_{\rm tel}\,\eta\,t\,\frac{\lambda}{hc},
  \label{eq:etcsig}
\end{equation}
where $A_{\rm tel}$ is the effective collecting area, and $\eta$ is the end-to-end throughput including optics, disperser, and detector quantum efficiency. Also $\lambda/hc$ converts line energy to photon number. In a slitless spectrograph a uniform sky disperses onto every detector pixel across the full band-limiting filter. The diffuse background collected per pixel per second is
\begin{equation}
  B_{\rm pix}=\bar i_{\rm sky}\,A_{\rm tel}\,\eta\,\Omega_{\rm pix}\,\Delta\lambda_{\rm filt}\,\frac{\lambda}{hc},
  \label{eq:etcbg}
\end{equation}
where $\bar i_{\rm sky}$ is the mean sky surface brightness per unit wavelength, $\Omega_{\rm pix}$ is the pixel solid angle, and $\Delta\lambda_{\rm filt}$ is the filter bandwidth that the proposal imposes to keep slitless traces (dispersed spectra) short. The line falls on $n_{\rm pix}$ pixels set by the source size along the cross-dispersion direction times the resolution-element width. Adding source shot noise, sky, dark current $D$, and read noise $\sigma_{\rm read}$ over $n_{\rm read}$ read-outs in quadrature gives the total variance
\begin{equation}
  N^2=S+B_{\rm pix}\,n_{\rm pix}\,t+D\,n_{\rm pix}\,t+n_{\rm read}\,n_{\rm pix}\,\sigma_{\rm read}^2,
  \label{eq:etcnoise}
\end{equation}
and the $5\sigma$ line-flux limit is the flux for which $S=5N$. For the space near-infrared background the sky term of Eq.~\eqref{eq:etcnoise} dominates the other three. The noise therefore reduces to $N\simeq(B_{\rm pix}\,n_{\rm pix}\,t)^{1/2}$. Setting $S=5N$ with $F_{\rm line}=F_{5\sigma}$ and inserting the signal of Eq.~\eqref{eq:etcsig} leads to
\[
...
\]
where the explicit factor of $t$ in the denominator combines with the $t^{1/2}$ in the numerator to leave the $t^{-1/2}$ dependence. Substituting the slitless background $B_{\rm pix}$ from Eq.~\eqref{eq:etcbg} and cancelling the common factor $A_{\rm tel}\,\eta\,(\lambda/hc)$ (present both inside and outside the square root) yields the closed form as,
\begin{equation}
...
Equation~\eqref{eq:etcbg} applies to the full-field slitless mode. The image-sliced mode has a different background because the pseudo-slit assigns one sky element to each spectral trace. For continuum flux density $f_\lambda$ extracted over a spatial solid angle $\Omega_{\rm ext}$, the background-limited signal-to-noise per resolution element is
\begin{equation}
 \left(\frac{S}{N}\right)_{\rm cont}
 \simeq
 \frac{f_\lambda\,[A_{\rm tel}\eta t(\lambda/hc)\Delta\lambda_{\rm res}]^{1/2}}
 {(i_{\lambda,{\rm sky}}\Omega_{\rm ext})^{1/2}}
 \propto R^{-1/2},
 \label{eq:snrcontR}
\end{equation}
where $\Delta\lambda_{\rm res}=\lambda/R$. The $R^{-1/2}$ factor remains for native-resolution continuum measurements even when an image slicer removes the full-filter background term. Binning the sliced spectrum to a fixed lower resolution recovers the photon-limited continuum S/N because the source and sky counts are summed over the same wavelength interval.

An unresolved emission line follows
\begin{equation}
 \left(\frac{S}{N}\right)_{\rm line}
 \propto \Delta\lambda_{\rm eff}^{-1/2},
 \qquad
 \Delta\lambda_{\rm eff}=
 \max\left(\frac{\lambda}{R},\Delta\lambda_{\rm int}\right),
 \label{eq:snrlineR}
\end{equation}
where $\Delta\lambda_{\rm int}$ is the observed intrinsic line width. Increasing $R$ improves the background-limited line S/N as $R^{1/2}$ while the line remains unresolved. The improvement stops after the instrumental profile becomes narrower than the intrinsic line. Detector noise, slicer throughput, scattered light, and spectral cross-talk reduce the ideal gain. Equations~\eqref{eq:snrcontR} and~\eqref{eq:snrlineR} prevent a continuum scaling from being applied to every high-resolution science case.

Table~\ref{tab:etc} lists the adopted baseline parameters. The mean zodiacal sky brightness near $1.6\,\mu$m is taken from the standard reference model \cite{leinert1998}. The collecting area follows from the $3.5$\,m aperture, and the detector terms are baseline values typical of near-infrared HgCdTe arrays. With these values the sky background is $B_{\rm pix}\simeq0.90$ e$^-$\,s$^{-1}$ per pixel. Over $t=0.75$\,hr exposure, the accumulated sky is $3.9\times10^{4}$ e$^-$ across the $n_{\rm pix}=16$ line footprint while the dark and readout contributions are only $4.3\times10^{2}$ and $4.8\times10^{3}$ e$^-$. This confirms that the measurement is background-limited. Equation~\eqref{eq:etc} then returns $F_{5\sigma}\simeq1.9\times10^{-17}\,{\rm erg\,s^{-1}\,cm^{-2}}$ at $0.75$\,hr.

This first-principles value is about five times deeper than the $1.0\times10^{-16}$ normalization adopted in Eq.~\eqref{eq:f5sigma}. The two agree in functional form and in order of magnitude. The offset is the expected planning margin. The empirical reference is deliberately conservative because it folds in throughput losses below the assumed $\eta$, larger emitting-region sizes which raise $n_{\rm pix}$, the wider filter bandwidths used in the most crowded fields, and the flux lost to deblending through the factor $C_{\rm blend}$ of the survey signal-to-noise relation of Section~\ref{sec:elgdepth}. Across the realistic ranges of these parameters, namely $\eta$ from $0.2$ to $0.4$, $n_{\rm pix}$ from $8$ to $40$, and $\Delta\lambda_{\rm filt}$ up to the full channel width, the calculator brackets $F_{5\sigma}$ from roughly $1\times10^{-17}$ to about $1\times10^{-16}$ at $0.75$\,hr, with the upper edge reached only for the most extended sources and the widest filters. The adopted depth therefore sits on the conservative side of the photon-limited range. Eq.~\eqref{eq:f5sigma} is a safe planning floor rather than an optimistic limit.
...
\caption{The open-source exposure-time calculator and its graphical output (\url{https://github.com/kjhan0606/3.5mST}). A telescope preset and an observing mode set the aperture and detector parameters. The imaging panel shows the $5\sigma$ point-source depth for Johnson--Cousins $UBVRI$, SDSS $ugriz$, and 2MASS $JHK_{\rm s}$ photometry at three exposure times. Each passband uses its pivot wavelength and rectangular-equivalent width. The lower panel gives the line-flux depth across the full wavelength range. The selector exposes $R=1000$ and $R=5000$ for resolution tests. The present $R=5000$ calculation represents a full-field slitless comparison. A flight prediction for the image-sliced mode requires the separate geometry and throughput inputs in Table~\ref{tab:etcmodes}.}
\label{fig:etcgui}
\end{figure}
...
\midrule
3.5\,m full-field slitless & $0.36$--$3.00\,\mu$m in band-limited settings & $R_{\rm point}=1000$ & Wide ELG redshift survey and broad-feature transient spectroscopy\\
3.5\,m image sliced & $0.36$--$3.00\,\mu$m design goal in band-limited settings & $R_{\rm point}=5000$ & Restricted-field line profiles and targeted galaxy dynamics\\
...
\end{tabularx}
\caption{Mode-specific wavelength comparison with Roman WFI. Roman imaging reaches $2.30\,\mu$m through F213 while Roman spectroscopy ends at $1.93\,\mu$m for G150 and $1.80\,\mu$m for P127. The Roman values come from the STScI WFI quick reference and the Roman technical-information release cited there \cite{romanwfi2025}. The $3.5$\,m image-sliced range remains a design goal whose detector coverage and throughput require a Phase A optical study.}
...
\caption{The left panel gives the imaging $5\sigma$ point-source depth in a $1$\,hr exposure for each mission in its own filters. The Roman sequence includes F213 and therefore reaches $2.30\,\mu$m in imaging. The right panel gives the slitless $5\sigma$ line-flux depth in a $0.75$\,hr exposure for the $3.5$\,m full-field $R=1000$ mode and Roman G150. Roman G150 ends at $1.93\,\mu$m. The Roman HLSS planning depth is marked. The plotted $3.5$\,m curve uses band-limiting $0.4\,\mu$m filters and does not represent the separate $R=5000$ image-sliced mode.}
...
The red cutoff follows from the stated cosmology experiment rather than from a general preference for infrared coverage. A survey advertised over $1<z<3$ must not change its tracer definition halfway through that volume. For a line with rest wavelength $\lambda_0$, a red cutoff $\lambda_{\rm cut}$ gives
\begin{equation}
...
\bottomrule
\end{tabularx}
\caption{Observed wavelengths of the principal ELG diagnostic lines at $z=3$, computed from $\lambda_{\rm obs}=\lambda_0(1+z)$. Rest wavelengths are from the NIST Atomic Spectra Database \cite{nistlines}. Ly$\alpha$ enters the $0.3\,\mu$m blue edge at $z=1.47$ and appears at $0.486\,\mu$m by $z=3$. Resonant transfer through neutral hydrogen changes the Ly$\alpha$ visibility and can shift the line centroid from the systemic redshift. Ly$\alpha$ is therefore a supplementary diagnostic rather than a guaranteed systemic-redshift line \cite{dijkstra2014}. A $2.0\,\mu$m cutoff contains [O\,II], H$\beta$, and the blue [O\,III] component at this redshift. The line placement alone does not guarantee that the weaker H$\beta$ line is detected. Calibrated throughput to $2.70\,\mu$m retains the independent H$\alpha$+[N\,II]+[S\,II] complex.}
...
\item \textbf{$2.70\,\mu$m science threshold.} All lines in Table~\ref{tab:z3lineplacement} remain in band through $z=3$. The H$\alpha$+[N\,II]+[S\,II] complex supplies an independent strong feature for redshift confirmation and provides line ratios that diagnose active nuclei and changing nebular conditions.
...
The transition at $2.0\,\mu$m is not only a loss of targets. Above $z\simeq2.05$, the catalog changes from a mixed Balmer and oxygen-line selection to a predominantly oxygen-line selection. The transition changes galaxy bias, redshift success, and completeness because H$\alpha$ is a recombination line tied to the ionizing massive-star population, while [O\,III] and [O\,II] respond strongly to metallicity and ionization state \cite{kennicutt1998,khostovan2015,saito2020}. The selection change occurs in the same redshift range where the mission aims to interpret the anisotropic clustering amplitude as growth rather than as an evolving tracer systematic. Retaining the Balmer-selected population also protects the effective number density $n(z)$ and the effective clustering volume defined later in Equation~\eqref{eq:veff}. A loss of one tracer is amplified when $nP$ approaches unity because the statistical weight scales as $[nP/(1+nP)]^2$.
...
The target population is a star-forming ELG with blue continuum, nebular recombination lines, and collisionally excited oxygen lines. The most important redshift features are [O\,II] $\lambda3727$, H$\beta$ $\lambda4861$, [O\,III] $\lambda4959,\lambda5007$, H$\alpha$ $\lambda6563$, and the neighboring [N\,II]/[S\,II] complex. At native $R=1000$, the [O\,III] doublet is cleanly separated for compact sources over the full cosmological range. H$\alpha$ and [N\,II] are often modeled as a blended complex for extended sources, but still provide a high-S/N redshift feature when combined with morphology priors and other lines.
...
\end{itemize}
The DESI-derived [O\,II] calculation below is therefore used only as a low-redshift empirical reference. The full 3.5 m all-line survey yield should be derived from a redshift-dependent LF plus selection-function model, then validated with image-level simulations and deeper calibration fields.
...
\end{equation}
where $V_{\rm c,deg^2}$ is the comoving volume per square degree in the redshift bin and $\overline{C}_k$ is the mean completeness correction in luminosity bin $k$. The conservative DESI table below sets $\overline{C}_k=1$. The medium-depth counts should therefore be read as lower limits rather than as a complete faint-end extrapolation. For an observed LF table, the predicted count distribution in one field of view is computed by summing the measured LF bins above the luminosity threshold,
\begin{equation}
...
\end{equation}
with $L_{\rm lim}=4\pi D_L^2(z_{\rm mid})F_{\rm lim}$. A single \fovlarge\ frame has area 0.25 deg$^2$ and, therefore, $N_{\rm frame}=0.25\Sigma$. Table~\ref{tab:desiLFcounts} summarizes this DESI-derived reference. It uses DESI [O\,II] only. It is a conservative reference for the proposed all-line slitless survey, not the full H$\alpha$+[O\,III]+[O\,II] yield. For $z>1.6$, DESI no longer provides the same direct [O\,II] calibration. The high-redshift extension should be tied to external [O\,II]/H$\alpha$ luminosity-function and number-count measurements \cite{comparat2015,saito2020,mehta2015}.
...
\end{equation}
This gives $m_{\rm AB}\simeq24.3$, 25.1, 25.8, and 26.6 mag for $t_{\rm tot}=0.75$, 3, 12, and 48 hr, respectively. The line-selected ELG catalog therefore reaches deeper in star-formation activity than in continuum mass completeness, which is why the medium and deep tiers are needed to calibrate selection.
...
\begin{equation}
  ...
  \right],
\end{equation}
without applying a $K$-correction. The black curve marks the effective eBOSS $r$-band limit, $r\simeq22.44$, retained only as a historical comparison line. The purple curve marks the DESI DR1 99th-percentile depth of the plotted secure ELG sample, $r\simeq24.08$. The DESI target selection is not a pure $r$-band cut. It uses a $g$-band magnitude cut and a $(g-r)$ versus $(r-z)$ color box. The plotted depth curves should be read as observed-frame indicators of catalog truncation rather than formal single-band selection boundaries \cite{raichoor2022}. The plotted catalog points are clipped at the adopted wide-tier continuum limit, $m_r\le24.3$. Galaxies fainter than the nominal target limit are not shown. DESI occupies the key $0.6<z<1.6$ range and nearly reaches the proposed wide-tier continuum depth. The proposed space survey extends this selection logic to $1\lesssim z\lesssim3$ using deeper medium/deep tiers and near-infrared emission lines.
...
\end{equation}
with the same $\Omega_{\rm eff}$ and cosmology used for the [O\,II] LF calibration above. Figure~\ref{fig:desiMlf} shows the resulting evolution over $0.6<z<1.6$. The turnover at faint $M_r$ is not the galaxy LF turnover. It is the DESI target-selection and redshift-success boundary. This is why the figure is useful for planning depth, but not sufficient by itself for extrapolating medium/deep-tier yields.
...
\bottomrule
\end{tabularx}
\caption{DESI-observed continuum count integral corresponding to Figures~\ref{fig:elgmagdepth} and \ref{fig:desiMlf}. The count is $N_{\rm frame}=0.25\,\Sigma$ after integrating the binned DESI observed-frame luminosity function brighter than the listed $M_r-5\log_{10}h$ limit. Because the local DESI catalog used here was clipped at $m_r\le24.3$, the medium and deep columns are lower limits and mainly flag where external deep surveys should supply the missing faint population.}
\label{tab:desiMcounts}
\end{table}

The required external calibration is therefore not optional. DESI gives the wide-area normalization and the $0.6<z<1.6$ color-selected ELG reference, while DEEP2, VVDS, and COSMOS-based emission-line catalogs supply the fainter continuum and higher-redshift information needed to model the DESI truncation boundary, color incompleteness, and faint-end LF slope, with the zCOSMOS group catalog adding environment and membership context \cite{newman2013deep2,lefevre2013vvds,saito2020,knobel2012zcosmos}. Literature [O\,II], H$\beta$+[O\,III], and H$\alpha$ LF measurements show strong redshift evolution in both $L^\star$ and normalization. The final yield model should be a joint fit to DESI plus these deeper samples rather than a pure extrapolation of the DESI observed-frame luminosity function \cite{comparat2015,comparat2016,khostovan2015,mehta2015}.
...
\bottomrule
\end{tabularx}
\caption{Illustrative ELG survey tiers. Exposure times are total science exposure per filter/band and include the required three orientations, but not slew, readout, dither, calibration, or scheduling overhead. The line-flux limit $F_{5\sigma}$ is a faint-detection threshold. A \emph{smaller} value therefore means a fainter line is still detected, i.e. greater depth. Because the survey is background-limited, $F_{5\sigma}\propto t_{\rm tot}^{-1/2}$. The longer-exposure tiers reach fainter limits and are deeper, not shallower: the wide tier at $3\times900$ s reaches $1.0\times10^{-16}$, whereas the ultra-deep tier at $3\times16$ hr reaches $1.3\times10^{-17}$, nearly an order of magnitude fainter.}
\label{tab:wedding}
\end{table}
...
\end{equation}
where $D_M$ is the transverse comoving distance and $r_d$ is the sound horizon at the drag epoch. By itself, therefore, the survey measures no absolute $H_0$. That value requires a sound-horizon calibration from the CMB, BBN, or another early-universe prior. Without one, the clean late-time observable is the inverse-distance-ladder combination $H_0r_d$.
...
The tracked detector-scene harness now performs a blind compact-line search from direct-image positions. The search scans the full first-order trace at all three rolls without a truth wavelength or truth line-flux input. It records H$\alpha$, [O\,III], and [O\,II] single-line hypotheses. Its frozen candidate list is matched to truth only after extraction and produces a provisional redshift-binned selection table. The compact-line search is a discovery baseline. It does not fit neighbouring spectra jointly or resolve the single-line ambiguity. It does not enter the science forecast.
...
The survey should be proven by simulated images, not by analytic exposure-time arguments alone. None of the purity, completeness, or flux-bias numbers quoted above as requirements has yet been demonstrated at survey scale. Figure~\ref{fig:crowded} illustrates the baseline deblending geometry and method on a single tile. Figure~\ref{fig:slicer} illustrates an image-slicer alternative. Neither yet provides the recovery statistics that would validate it. The joint sparse inversion's computational cost at the scale of the full survey (roughly $10^6$--$3\times10^6$ objects across $10^4$ roll-visits) has not yet been characterized. The Phase A validation suite should inject realistic galaxies into detector scenes, recovering redshifts with the same pipeline planned for flight. The required outputs, which this proposal treats as requirements to be verified rather than results in hand, are:

The first validation realization uses the tracked program \texttt{detector\_scene\_validation.py}. The program writes an input catalog containing position, redshift, continuum magnitude, size, axis ratio, line equivalent width, and line flux. The program also writes independent noisy direct and $0^\circ/45^\circ/90^\circ$ dispersed exposures and an imperfect direct-image source catalog with missed objects, centroid errors, and spurious detections. The present forced-line measurement checks the detector geometry and the association between input and recovered sources. The present calculation is not a redshift measurement and does not enter any science forecast. The following stage uses the calibrated sparse multi-roll forward-model extraction of Eq.~\ref{eq:scene}, which runs without input positions or wavelengths as priors.

A verification run now exercises the key science-A geometry with a $1536\times1536$ pixel tile and $1091$ pixel traces. It uses three roll angles and two dithers per roll. The run writes truth and direct-image catalogs, detector realizations, and recovery tables binned by magnitude and nearest-neighbour separation. It verifies that the proposed data products preserve the required association between truth and recovery across the complete observing pattern. Its analytic source population and forced-line measurement are intentionally unsuitable for a survey performance claim. The Phase A acceptance run must use a calibrated deep-field catalog and detector calibration files. It must then apply a blind redshift search with the flight forward-model extractor.

A second verification run converts a 3D-HST COSMOS catalog into the same detector geometry \cite{skelton2014,momcheva2016}. The input contains 907 sources to $F160W_{\rm AB}=26$. It retains measured positions, magnitudes, half-light radii, axis ratios, position angles, reference redshifts, and fitted line properties. The reference redshifts and the ETC conversion from line flux to detector electrons limit this run to a test of scene density and data products. It does not test blind redshift recovery or survey sensitivity. The Phase A acceptance run must replace both with a calibrated detector model and a blind flight-pipeline extraction.
...
The two high-Galactic-latitude caps that define the wide survey were chosen not only to avoid the Milky Way but to enclose the richest legacy deep fields in the sky (Figure~\ref{fig:skymap}). The north cap contains COSMOS, GOODS-N, the Extended Groth Strip, and the Subaru Deep Field. The south cap contains GOODS-S with the Hubble Ultra Deep Field together with the UDS/SXDS and XMM-LSS fields. These fields hold decades of deep HST and Subaru imaging, extensive ground and space spectroscopy, and now JWST coverage from CEERS in the Extended Groth Strip, JADES in GOODS-S, and COSMOS-Web. The wide survey therefore inherits an unusually complete set of ancillary data over its own footprint. Within each cap the $100$--$300$ deg$^2$ wide tier and its nested medium and deep tiers are located on a high-Galactic-latitude legacy concentration, the NDWFS/HETDEX Bo\"otes field ($b\simeq+67^\circ$) in the north and the SXDS/UDS/XMM-LSS deep field ($b\simeq-59^\circ$) in the south (Figure~\ref{fig:deepzoom}). Both were chosen at high $|b|$ to stay clear of Galactic extinction and cirrus. The Bo\"otes reference is particularly valuable because the HETDEX blind emission-line survey covers the same field, giving a direct external comparison sample for the slitless emission-line selection while the remaining legacy fields lie elsewhere in the same caps and contain additional deep and calibration pointings.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{elg_skymap.png}
\caption{Proposed ELG survey fields on an equatorial Mollweide projection, background the real Planck 857\,GHz thermal-dust map \cite{planck2018hfi} (native $\sim$5$'$ resolution, CDS \textit{hips2fits}) with its intensity scale below. The north and south Galactic caps ($|b|\gtrsim45^\circ$, green and blue outlines) avoid the Milky Way zone ($|b|<20^\circ$, red curves) and lie within the DESI, Euclid, Rubin/LSST, and Roman footprints. Orange open boxes mark the two ELG wide-tier footprints at true scale ($\sim$150\,deg$^2$ each), both in dark low-cirrus windows. Cyan squares mark the two ultra-deep imaging fields, the Planck-minima references of Figure~\ref{fig:imgfields}, shown as symbols since at $2.25$\,deg$^2$ they are far smaller than the ELG tiers. Red diamonds mark the two SN Ia time-domain cadence fields ($\sim$5--10\,deg$^2$ each), nested inside each wide tier so SN hosts inherit real slitless redshifts from the flagship survey (zoomed in Figure~\ref{fig:deepzoom}). Magenta outlines mark the imaging-only wide-extension fields ($\sim$306\,deg$^2$ per cap), beside the Lockman Hole in the north and the south Galactic pole in the south. Gold stars mark legacy HST and Subaru deep fields within the caps (Figure~\ref{fig:deepzoom}).}
\label{fig:skymap}
\end{figure}
...
\midrule
\multicolumn{4}{@{}l}{\textit{North reference: NDWFS/HETDEX Bo\"otes, $b\simeq+67^\circ$, $\sim$9.3 deg$^2$}}\\
...
\midrule
\multicolumn{4}{@{}l}{\textit{South reference: SXDS/UDS/XMM-LSS, $b\simeq-59^\circ$}}\\
...
The northern reference is unusual because HETDEX \cite{hetdex2021} has already conducted a blind emission-line survey over the same sky, detecting Lyman-$\alpha$ emitters at $1.9\lesssim z\lesssim3.5$ and [O\,\textsc{ii}] emitters at $z\lesssim0.5$ with no imaging preselection. This is the one external data set that is selected the same way our survey is, by emission line rather than by continuum. It therefore measures the completeness and purity of an emission-line catalog directly and calibrates the [O\,\textsc{ii}]--Ly$\alpha$ confusion, which is the principal interloper channel at the survey edges. The synergy goes beyond a one-way cross-check because the two surveys barely overlap in wavelength. VIRUS records only $3500$--$5500$\,\AA. A HETDEX Ly$\alpha$ emitter at $z\simeq2.3$ places its [O\,\textsc{iii}] line at $1.65\,\mu$m and its H$\alpha$ line at $2.17\,\mu$m, squarely inside the near-infrared settings of this mission. The same galaxy is measured through disjoint sets of lines. The calibration therefore runs in both directions. The rest-frame optical lines of this survey enable us to settle the Ly$\alpha$-versus-[O\,\textsc{ii}] classification, which is the dominant HETDEX systematic. HETDEX in return confirms the blue-end single-line redshift solutions of the slitless catalog. Over the shared $2\lesssim z\lesssim3.5$ volume the two samples also trace the same large-scale structure with different tracers, instruments, and selection functions. Their cross-correlation suppresses cosmic variance in the multi-tracer sense \cite{mcdonaldseljak2009} and exposes selection systematics that neither autocorrelation isolates. Pairing the HETDEX Ly$\alpha$ fluxes with the Balmer lines of the same objects finally yields object-by-object Ly$\alpha$ escape fractions across the Bo\"otes footprint, a transparent-IGM baseline at $2\lesssim z\lesssim3.5$ against which the reionization-era Ly$\alpha$ statistics of Section~\ref{sec:lyaforest} are interpreted. The deep continuum and spectroscopic layers of the same field, NDWFS optical and near-infrared \cite{jannuzi1999}, SDWFS mid-infrared \cite{ashby2009}, and the $\sim$23{,}000 AGES redshifts \cite{kochanek2012}, then supply the photometric-redshift and stellar-mass priors. The XBo\"otes Chandra mosaic \cite{murray2005} together with the LOFAR deep radio map \cite{tasse2021} flags active nuclei and obscured star formers that would otherwise contaminate the line-flux statistics.

The southern reference contributes complementary strengths, depth in the near-infrared and image quality for galaxy shapes. UKIDSS UDS \cite{lawrence2007}, the deepest wide-area near-infrared field on the sky, pins the stellar continuum and photometric redshifts of faint galaxies well below the wide-tier line-flux limit. VIDEO \cite{jarvis2013} then extends that coverage across the full medium-tier area. XMM-SERVS \cite{chen2018} supplies the X-ray active-nucleus census over the wide tier. Its sources are flagged out of the line-flux statistics before they bias the luminosity functions and receive slitless redshifts and line classifications in return. The HSC-SSP UltraDeep imaging \cite{aihara2018} delivers weak-lensing-quality shapes and the deepest optical photometry, reused by the diffuse-light and cluster-lensing programs of Section~\ref{sec:lsb} for the mass cross-check, gaining in return the dense emission-line redshifts that calibrate its photometric-redshift and weak-lensing calibration. Deep VIMOS spectroscopy from VANDELS \cite{mclure2018} reaches fainter than the ELG threshold supplying the continuum-selected redshift reference sample that plays the same role as HETDEX plays in the north, testing the slitless solutions on galaxies selected the opposite way. The field's planned Subaru Prime Focus Spectrograph \cite{takada2014} and MOONS observations will keep extending that table over the mission lifetime.

The synergy runs in both directions because the southern assets are continuum-selected, the exact complement of the slitless line selection. The deep $JHK$ photometry finds the massive, quiescent, and dusty galaxies whose lines are too weak for the survey, measuring the incompleteness of the emission-line census from the continuum side, population by population. The slitless spectra in return supply redshifts and line fluxes for the continuum-faint, high-equivalent-width dwarfs that no continuum-selected sample reaches. Pairing a line flux from this survey with a stellar mass from UDS and VIDEO places every common galaxy on the star-formation-rate versus stellar-mass plane, where the joint property distributions $n(\boldsymbol{\theta}|z)$ and the line completeness factors $C_i$ of Section~\ref{sec:elgdepth} are calibrated. An equivalent width is the ratio of a line flux to a continuum level. The survey supplies the line flux while UDS and VIDEO supply the background continuum. The equivalent width can therefore only be measured by combining the two data sets.

Taken together the two references give the program three things that a single field does not. First, they combine an emission-line-selected reference sample in the north with deep continuum redshifts in the south. Both failure modes of the slitless selection, missing real lines and misidentifying interlopers, are calibrated against independent external data. Second, the panchromatic coverage in both fields supplies the morphology and size priors that set the blending factor $C_{\rm blend}$ of Eq.~\eqref{eq:snline} and the line-ratio and continuum-magnitude conversions behind the luminosity-function counts of Tables~\ref{tab:desiFOVsum} and~\ref{tab:desiMcounts}. The count model of Section~\ref{sec:elgdepth} is anchored on measured rather than assumed inputs. Third, the northern field near right ascension $14^{\rm h}30^{\rm m}$ and the southern field near $2^{\rm h}20^{\rm m}$ sit in opposite hemispheres, giving year-round scheduling, two statistically independent volumes for cosmic-variance control, and a cross-hemisphere test of instrumental and selection systematics. Both fields are also deep-drilling targets for Rubin, Euclid, and Roman. The reference placement keeps the survey inside a coordinated multi-facility footprint from the first pointing.
...
The method combines two mission data products. Diffraction-limited imaging constrains the Einstein radius $\theta_E$, the projected mass inside the radius, and the radial slope $\gamma'$ from the reconstructed ring. The $R=1000$ slitless spectrum adds an aperture-integrated estimate of the deflector stellar velocity dispersion $\sigma$. Its $\sim300$\,\kms\ point-source resolution limits low-dispersion systems. Lens-selected deflectors are commonly massive ellipticals with $\sigma\gtrsim250$\,\kms. Template fitting and morphology deconvolution across three roll angles may recover an integrated dispersion for population analyses. The $R=5000$ image-sliced mode reduces the point-source width to about $60$\,\kms\ and provides the preferred mission measurement for a selected lens sample. The slice geometry still does not guarantee spatially resolved kinematics. Flagship systems that require two-dimensional stellar-velocity fields need external integral-field spectroscopy \cite{tdcosmo2025}.
...
With $\sim$$10^5$ lenses expected from imaging surveys, both lens identification and mass modeling should be automated. This is the technical core of the program (Figure~\ref{fig:lenspipe}). For \emph{finding}, convolutional and transformer classifiers operate on the stable-PSF imaging. Such networks already beat human visual inspection in controlled tests \cite{metcalf2019}, have produced hundreds of candidates from millions of catalog sources in ground-based data \cite{jacobs2019} and inform the discovery engines now releasing space-based lens catalogs \cite{walmsley2025}. A complementary spectroscopic channel flags lenses by detecting background emission lines superimposed on a foreground red galaxy in the grism data, yielding a redshift at the moment of discovery.

For \emph{modeling}, classical Markov-chain lens reconstruction costs days to weeks per system and does not scale to $10^5$ lenses. Neural and simulation-based inference (SBI) removes this bottleneck. Convolutional estimators are $\sim$$10^7$ times faster than maximum-likelihood modeling \cite{hezaveh2017}, Bayesian networks return calibrated parameter posteriors \cite{perreaultlevasseur2017}, and population studies show that automated per-lens posteriors with $\sim$9\% precision combine to sub-percent $H_0$ over a few hundred lenses without detectable bias \cite{park2021}. We adopt simulation-based inference (neural posterior estimation), trained on mission-specific PSF and noise realizations, to infer per-lens parameters ($\theta_E$, $\gamma'$, ellipticity, $\kappa$). A hierarchical Bayesian model then jointly infers the cosmological parameters and the population-level nuisance distributions (density slope, anisotropy, and the mass-sheet), while marginalizing each lens \cite{wagnercarena2021,legin2023}. The spectroscopic redshifts and dispersions enter as informative priors that sharpen the per-lens posteriors and calibrate the distance scale. The same hierarchical framework absorbs the DSPL $\beta$ measurements and the $\gamma'(z)$ population as internal constraints. Therefore, the strong-lensing cosmology is delivered as one self-consistent inference rather than a heterogeneous lens-by-lens collection.

This pipeline poses a validation burden that the deblending forward model of Section~\ref{sec:deblending} does not. Lens finding is the more mature half, with the cited classifiers already validated on real ground- and space-based imaging rather than only simulations. Mass modeling is the less mature half. The neural posterior estimator is necessarily trained on simulated PSF and noise realizations since no flight data yet exist, and amortized simulation-based inference of this kind is known to become overconfident when the training and test distributions diverge. Before any per-lens or hierarchical posterior enters the cosmology sample, the estimator and the hierarchical model should be validated against lenses already confirmed by precursor surveys and against injection tests that withhold this mission's own roll diversity and PSF. This is the same discipline Section~\ref{sec:deblending} applies to the deblending priors. Automated inference is the engine of the modeling step but not the judge of it. That role belongs to the injection-recovery validation the modeling step should pass before its posteriors are trusted.
...
Environment is what disentangles them. A dwarf in a cluster has been tidally stripped, ram-pressure swept, and harassed. A dwarf in a group has likely interacted with neighbors. Any inference about dark-sector inner structure from such systems should separate these environmental effects from the signal because tidal stripping can lower a halo's central density in a way that superficially resembles a self-interaction core. Voids provide a low-density comparison sample rather than a zero-interaction control. The Void Galaxy Survey finds predominantly gas-rich, low-luminosity blue disks. Many systems also show signs of ongoing gas accretion or interactions, which indicates that assembly continues inside voids \cite{kreckel2012}. A MaNGA comparison finds no statistically significant environmental difference in the dark-matter-to-stellar-mass ratio or in its relation to gas-phase metallicity \cite{douglass2019}. These results make the controlled comparison itself the measurement. They do not justify assuming that void dwarfs have uniformly larger dark-matter fractions. Their gas-rich disks remain valuable rotation-curve targets for the core--cusp distinction and the diversity problem of Figure~\ref{fig:sparcdiv}. Because the mission observes void, field, group, and cluster dwarfs with the same instrument and selection function, it places all four environments on a common footing and measures the environmental dependence of both the galaxy--halo connection and the inner density structure without cross-survey systematics. This environmental span also sets the faint end of the galaxy--halo connection at the threshold of galaxy formation, where the relation between stellar and halo mass steepens and scatters. It tests whether a minimum halo mass for galaxy formation exists and how sharply \cite{simon2019,bullock2017}. Present heterogeneous samples for this measurement are dominated by exactly the selection systematics that a single controlled survey removes.
...
\midrule
\textbf{ELG BAO/RSD}\newline Flagship & Concurrent imaging and 3-roll $R=1000$ slitless spectroscopy & 100--300 \degti\ with $10^6$--$3\times10^6$ redshifts & $F_{5\sigma}=10^{-16}$ wide and $2.5\times10^{-17}$ deep & \textcolor{green}{Core mission}\\
...
\textbf{LSB blind census}\newline Survey by-product & Imaging during the flagship tiers & 100--300 \degti\ with $\gtrsim$500--1400 LSBGs and UDGs at the published DES density & $\mu_{5\sigma}(10'',g)=29.8$--$31.3$ from concurrent imaging & \textcolor{green}{Proceed}\\
...
\textbf{Imaging wide extension}\newline Major companion & Imaging only & 612 \degti\ across the Lockman Hole and south-polar fields & 0.25 hr per tile in simultaneous $g+H$ with $\mu_{5\sigma}(10'',g)=29.2$ & \textcolor{green}{Proceed}\\
...
\midrule
\textbf{ELG line profiles}\newline Calibration & Targeted $R=5000$ image-sliced spectroscopy & Compact ELGs across redshift, size, and line-flux bins & Sliced-mode ETC with measured throughput & \textcolor{gold}{Phase A validation}\\
\textbf{High-$z$ quasars}\newline Major companion & Targeted $R=1000$ slitless spectroscopy & 50--100 quasars at $z>6.5$ & $3\times2$--$3\times5$ hr per target & \textcolor{green}{Proceed}\\
\textbf{Ly$\alpha$ forest}\newline Calibration & Targeted $R=1000$ slitless spectroscopy & 3000--5000 quasars at $2.1<z<3.5$ & 1--6 hr for $m_{\rm AB}\lesssim22.5$ & \textcolor{gold}{Moderate claims}\\
\textbf{Strong lenses}\newline Pathfinder & Discovery imaging and targeted $R=5000$ image-sliced spectroscopy & 20--30 clean systems & Sliced-mode exposure with external IFU support where required & \textcolor{gold}{Ancillary}\\
\textbf{LBG topology}\newline Ancillary & Targeted $R=1000$ slitless spectroscopy & 100--200 bright systems in overdensities & $3\times5$--$3\times10$ hr & \textcolor{rose}{Do not flagship}\\
\bottomrule
\end{tabularx}
\caption{Program-level numerical scope. Exposure entries for the full-field slitless mode include the required three orientations but exclude operational overhead. The $R=5000$ entries remain conditional on the image-sliced ETC inputs in Table~\ref{tab:etcmodes}. The observing-mode column excludes serendipitous supernova discoveries from repeat visits in other imaging tiers because the proposal has not forecast that rate.}
\label{tab:triage}
\end{table}
...
The observing bundle is defined at the detector level. Each grism setting obtains direct imaging, $0^\circ$, $45^\circ$, and $90^\circ$ dispersion views, and at least two sub-pixel dithers at each roll. The validation baseline uses two 900 s integrations per roll. The split supports bad-pixel rejection and cosmic-ray identification without changing the quoted planning depth. The final integration pattern remains contingent on the selected ramp-read mode and measured in-orbit cosmic-ray rate.
...
\textbf{Mission phase} & \textbf{Nominal period} & \textbf{Science time} & \textbf{Main deliverable}\\
 & mission year & hr & \\
\midrule
Commissioning and reference fields & Year 1 & 300--500 hr & Wavelength solution, point-spread-function and trace model, detector calibration package, 0/45/90 blind-recovery validation, image-slicer optical validation, and ultra-deep reference fields.\\
Wide ELG BAO/RSD & Years 1--4 & 1200--2500 hr & 100--300 \degti ELG redshift map with survey masks and random catalogs.\\
Medium/deep ELG tiers & Years 2--5 & 600--1200 hr & Completeness, purity, luminosity-function, overlap-calibration transfer functions, and tier-specific $P_{\rm secure}$.\\
...
Pathfinders and contingency & Years 3--5 & 300--700 hr & Image-sliced ELG calibration, strong-lens targets, LBG spectroscopy, ICL cluster imaging, and special-field programs.\\
\bottomrule
\end{tabularx}
...
The \fovsmall\ mode serves deeper fields where the science is not area limited. The targets include supernova cadence fields, the intracluster-light program of Section~\ref{sec:lsb}, deep calibration fields, quasar monitoring, and recovery tests in fields with existing spectroscopy. The smaller field should not define the wide RSD survey unless the mission accepts a much smaller cosmological footprint.

\subsection{Image-Sliced Target Mode}

The $R=5000$ mode begins with a direct image that defines target positions and the orientation of extended sources. The telescope then places the selected region on the slicer. The slicer reformats the region into a pseudo-slit and the disperser records one or more band-limited settings. Compact ELGs provide the first calibration sample because their [O\,\textsc{ii}], H$\beta$, [O\,\textsc{iii}], and H$\alpha$ profiles permit direct comparison with external spectroscopy. Strong-lens deflectors provide a second sample for stellar absorption features. The observing sequence records at least two dithers across the slice boundaries and repeats an orientation when the source major axis would otherwise dominate the line-spread function.

The mode advances from demonstration to scheduled observations after four tests. The optical model must fit the target field and full spectral trace on the detector. Calibration exposures must constrain wavelength and line-spread-function changes across slices. Crowded-scene injections must recover line flux and width without significant bias. The measured throughput and detector background must reproduce the dedicated ETC within the adopted uncertainty. Until those tests pass, $R=5000$ supplies no ELG yield, survey-volume increase, or observing-time reduction in the proposal.

\clearpage
\section{Flat Fielding and the Cirrus Foreground in the Imaging Surveys}\label{sec:flatcirrus}

Deep surface photometry is a mission-wide concern rather than a property of any single program. The same two systematic terms govern the direct imaging of the wide ELG tiers, the imaging-only wide extensions, the two ultra-deep fields, and the targeted cluster program of Section~\ref{sec:lsb}. The control strategy is collected here once with the ultra-deep fields as the most demanding case.

Surface photometry at $\mu\gtrsim29$ mag\,arcsec$^{-2}$ is limited by two instrumental-and-foreground terms that should be controlled together, the multiplicative flat field and the additive diffuse foregrounds. The flat-field requirement is severe because the science signal sits so far below the sky. A residual illumination error of one percent on the $\mu_g\simeq22$ mag\,arcsec$^{-2}$ zodiacal background imprints a false diffuse signal at $\mu_g\simeq27$, more than four magnitudes above the ultra-deep limit of $31.5$. The low-order flat should be controlled to the $10^{-4}$ level over the field.

The techniques that reach this level are established across the deep surveys. They share one principle, that the trusted low-order reference is the sky and the stars rather than any internal lamp because a lamp or dome screen illuminates the optics differently from the sky and imprints its own scattered-light pattern. The oldest of these methods is the star flat, in which repeated photometry of the same stars stepped across the focal plane solves directly for the position-dependent photometric response \cite{manfroid1996}. It remains the working correction of a modern wide-field mosaic since the Dark Energy Camera detrending applies exactly such star-flat solutions on top of its dome flats, using the internal consistency of on-sky observations to separate quantum efficiency, pixel-size variation, and stray light \cite{bernstein2017pasp}. The SDSS generalised the idea to the survey scale as photometric self-calibration (\"ubercalibration), a single least-squares solution over all overlapping scans that reached one-percent relative calibration in $griz$ over $8500$\,deg$^2$ \cite{padmanabhan2008}. Survey-design simulations by Holmes et al. found that robust global calibration requires repeated observations to place each source on widely separated detector regions \cite{holmes2012}. For the diffuse regime itself the working reference is the dark night sky. The deepest wide-aperture ground photometry to date, $\mu_r=31.5$ mag\,arcsec$^{-2}$ ($3\sigma$, $10\times10$\,arcsec$^2$) at the 10.4-m GTC, was flattened with dark-sky flats built from the dithered science exposures themselves \cite{trujillofliri2016}. At the hardware end, tunable monochromatic illumination measured against calibrated photodiodes transfers a laboratory throughput standard to the full telescope-plus-detector system, the route proposed for percent-level ground-based photometry \cite{stubbs2006}.
...
The Galactic cirrus enters differently as a sky-fixed additive foreground rather than a calibration error. It is not negligible even in the cleanest fields. At the field-mean $857$\,GHz intensities of the chosen ultra-deep fields, the scattered-light scaling of Section~\ref{sec:etc} predicts diffuse Galactic light at $\mu_g\simeq27.2$ mag\,arcsec$^{-2}$ using the measured optical $b(\lambda)$ ratios \cite{ienaka2013}. The corresponding near-infrared estimate is $\mu_H\simeq26.7$ mag\,arcsec$^{-2}$, several magnitudes brighter than the survey limit. The spatial structure of the cirrus therefore sets a more important confusion floor than its mean level. The footprint-scale thermal-dust structure of the selected fields is shown in Figure~\ref{fig:imgfields}, and their locations relative to the wider survey are shown in Figure~\ref{fig:skymap}. These maps define the field-selection prior. They are not assumed to provide a perfect pixel-by-pixel prediction of optical scattered light.

The control strategy has three stages. Field selection provides the largest reduction because the cirrus fluctuation power scales approximately as the cube of the mean brightness \cite{gautier1992}. Selecting fields at one quarter of the median sky brightness suppresses that power by a factor of order fifty relative to a typical high-latitude field. The Planck $857$\,GHz and IRAS $100\,\mu$m maps then provide a low-order spatial template. Its amplitude and wavelength dependence are fitted jointly with the survey images rather than fixed in advance because dust temperature, grain properties, and scattering geometry vary across the sky. The measured $b(\lambda)$ ratios provide informative priors for this fit \cite{ienaka2013}. Residual diffuse detections are finally assessed with the five survey bands. Deep optical measurements show that cirrus and stellar populations occupy different preferred colour ranges \cite{roman2020}, but colour is used as a probabilistic diagnostic rather than a hard separator. Each low-surface-brightness candidate receives a cirrus-contamination probability based on its correlation with the far-infrared template, morphology, and five-band spectral energy distribution. Injection and recovery tests on the real field backgrounds calibrate the resulting completeness and false-positive rate. This treatment addresses the known tendency of cirrus knots to enter ground-based low-surface-brightness catalogs \cite{zaritsky2023} without assuming that one colour cut removes every contaminant.
...
\textbf{The \facility\ will conduct a band-limited, multi-roll, $R=1000$ slitless survey optimized for ELG BAO/RSD cosmology. A restricted-field $R=5000$ image-sliced mode will provide targeted line-profile and galaxy-dynamics measurements after end-to-end optical validation. Supernova, high-redshift quasar, low-surface-brightness, and dwarf-galaxy programs exploit the same stable robotic observatory.}
\end{leadbox}
