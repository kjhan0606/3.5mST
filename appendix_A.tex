...
\begin{tikzpicture}[font=\sffamily]
  \foreach \x/\title/\val/\col/\vfont in {
    ...
    4.4/{Velocity resolution}/{300 \kms}/blue/\large,
...
The mission design is centered on native $R=1000$ slitless spectroscopy because the leading cosmological measurements are controlled by survey volume, line identification, source confusion, and selection-function calibration. The highest-leverage observing mode is a repeatedly-calibrated wide-field map of emission-line galaxies, not targeting precision internal kinematics for individual extended sources.
...
The filter plan is shaped by two properties of grism dispersion. First, a grism disperses at nearly constant $\Delta\lambda$ per pixel rather than constant $R=\lambda/\Delta\lambda$. Hence the resolving power rises linearly across any one setting. The quoted $R=1000$ refers to the centre of each setting. Second, a grating produces overlapping orders because second-order light at $\lambda$ lands at the same detector position as first-order light at $2\lambda$. At most one octave, $\lambda_{\max}\leq2\lambda_{\min}$, can therefore be recorded free of cross-order contamination. The survey therefore observes each arm through two band-limiting filters, each obeying the octave limit. On the optical arm they cover $0.36$--$0.70$ and $0.50$--$1.0\,\mu$m, on the near-infrared arm $1.0$--$1.75$ and $1.6$--$3.0\,\mu$m. This is the same order-sorting scheme used by the HST, JWST, Euclid, and Roman slitless instruments. These are the two settings per channel used to compute the exposure times and lay out the survey schedule in this document. Within each setting the resolving power stays between $R\simeq670$ and $1330$ around the central $R=1000$. A first-order trace (the dispersed spectrum) occupies about $1090$--$1330$ pixels assuming two pixels per resolution element. No transmitted wavelength has a second order inside the band of its own setting.

The choice of $R=1000$ is also robust against the naive suggestion of dispersing at higher resolution and binning the data back down. A slit and a slitless spectrograph respond to that binning in opposite ways. In a slit spectrograph the sky background is dispersed along with the source. Dispersing at higher resolution and binning back down therefore loses almost nothing because the sky per pixel drops just as fast as the source spreads out. However, slitless mode is different. With no slit, every pixel collects the sky, whatever the dispersion. A grism of five times the resolution spreads each source over five times as many pixels, yet the sky per pixel does not change because the same dispersion also brings five times as many neighbouring sky patches onto each pixel. Binning back down then sums five times the sky under the same source, and that background cannot be removed. The signal-to-noise of a binned quantity therefore degrades as the square root of the resolution ratio in the background-limited regime. For $R=5000$ binned back to $R=1000$ this is a factor of $5^{1/2}\simeq2.2$, equivalent to five times the exposure. The exposure-time calculator of Section~\ref{sec:etc} confirms the scaling directly. Raising the dispersion from $R=1000$ to $5000$ drops the binned synthetic-band S/N of a peak $z=1$ supernova from $12.2$ to $5.4$ in the same two-hour exposure. The emission-line depth for an extended source, by contrast, is identical at the two resolutions because the effective resolution is already set by the source size (Section~\ref{sec:effres}). The five-fold growth of the trace-overlap rate in crowded fields and of the detector pixel count per setting also remains since binning can undo neither. Higher resolution is therefore negative profit for the survey programs. On the other hand, the narrow-line science that would genuinely profit from higher resolution, such as the Ly$\alpha$ forest toward bright quasars, is better served by a small dedicated high-resolution mode than by raising the resolution of the whole survey. This single, fixed resolution is deliberately not changed for each companion program. Several of these programs involve broad-feature science which $R=1000$ already resolves comfortably, or even over-resolves, at no extra cost. Type~Ia supernova classification bins the native dispersion down to $R\simeq100$--$500$ without losing information (Section~\ref{sec:snia}), and the reionization damping wings and troughs are broad enough to need no higher resolution at all (Section~\ref{sec:lyaforest}). A few programs genuinely require more resolution than the native $R=1000$ provides. Rather than pushing the whole survey to a higher resolution, they are designed to work within that limit. The Ly$\alpha$ forest above requires a dedicated $R\gtrsim30{,}000$ targeted mode, and strong-lens deflector dynamics (Section~\ref{sec:stronglens}) is restricted to an aperture-integrated, population-level velocity dispersion rather than a spatially resolved one. The pattern is consistent throughout. $R=1000$ is set by the flagship ELG survey, and each companion program is tailored to what that choice actually delivers for it, not the reverse.
...
\bottomrule
\end{tabularx}
\caption{Observed wavelengths of the principal rest-optical ELG lines at $z=3$, computed from $\lambda_{\rm obs}=\lambda_0(1+z)$. Rest wavelengths are from the NIST Atomic Spectra Database \cite{nistlines}. A $2.0\,\mu$m cutoff contains [O\,II], H$\beta$, and the blue [O\,III] component at this redshift, but line placement alone does not guarantee that the weaker H$\beta$ line is detected. Calibrated throughput to $2.70\,\mu$m retains the independent H$\alpha$+[N\,II]+[S\,II] complex.}
...
\item \textbf{$1.30\,\mu$m cutoff.} The cutoff cannot execute the stated BAO and RSD survey. [O\,III] leaves the band above $z=1.60$ and H$\beta$ leaves above $z=1.67$. Only [O\,II] remains between $z\simeq1.67$ and $2.49$, and none of the principal lines remains over $2.49<z<3$.
...
The broad wavelength coverage is required to secure multiple-line redshifts across the full survey volume. Calibrated throughput to $2.70\,\mu$m retains the H$\alpha$+[N\,II]+[S\,II] complex at $z=3$ while the $3.0\,\mu$m operational band edge keeps the complex away from throughput roll-off and calibration losses. The extended redshift reach is added capability rather than the basis of the threshold. Phase A shall compare $1.30$, $2.00$, $2.70$, and $3.00\,\mu$m cutoffs with the same luminosity functions, ETC, deblending model, and survey time. The decision data products are $n(z)$, secure-redshift fraction, line-confusion rate, $nP$, $V_{\rm eff}$, and forecast uncertainties in $D_M/r_d$, $Hr_d$, and $f\sigma_8$.
\end{leadbox}
...
\caption{A halo from the mission team's cuRAMSES wave-dark-matter simulation at $z=1.49$ with $m=10^{-22}$\,eV. The panels show direct density slices through the densest level-11 wave cell over widths of $1.2$\,cMpc\,$h^{-1}$, $300$\,ckpc\,$h^{-1}$, and $75$\,ckpc\,$h^{-1}$. The finest leaf-cell scale present in the output is $0.59$\,ckpc\,$h^{-1}$. Density is computed as $|\psi|^2$ and is divided by the volume-averaged wave-dark-matter density. The slice displays the resolved interference field and the central density enhancement expected around a collapsed FDM halo \cite{schive2014}. A soliton classification is not imposed without a profile fit.}
\label{fig:fdmslice}
\end{figure}
...
\textbf{Mission phase} & \textbf{Nominal period} & \textbf{Science time} & \textbf{Main deliverable}\\
 & mission year & hr & \\
\midrule
Commissioning and reference fields & Year 1 & 300--500 hr & Wavelength solution, PSF/trace model, detector calibration package, 0/45/90 blind-recovery validation, ultra-deep reference fields.\\
