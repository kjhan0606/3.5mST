...
The spectrograph is a slitless two-arm system. A dichroic beam splitter sends 0.3--1.0\,$\mu$m light to the optical arm and 1.0--3.0\,$\mu$m light to the near-IR arm. Each arm uses band-limiting filters and a grism/prism disperser so that a crowded field does not produce decade-long spectra. It is desirable to observe with both arms simultaneously. Otherwise a revisit to the tile should be scheduled with the same orientation and pointing. Because the near-infrared arm reaches $3.0\,\mu$m, its optics should be cooled so that the telescope thermal self-emission stays below the zodiacal background at the reddest wavelengths. The exposure-time calculation of Section~\ref{sec:etc} sets this requirement, with the optics reaching the background-limited floor across the full $1$--$3\,\mu$m band for an operating temperature $\lesssim200$\,K.
