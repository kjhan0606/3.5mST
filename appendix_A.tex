...
\begin{equation}
  ...
  \label{eq:scene}
\end{equation} 
where $\mathbf{d}_{r}$ is the detector image at roll/orientation $r$, $\mathbf{s}$ is the set of object spectra, and $\mathbf{A}_{r}$ is the orientation-dependent projection operator built from the direct-image catalog, morphology model, trace solution, PSF, and wavelength calibration.

In practice the two-dimensional detector frame is flattened into a data vector $\mathbf{d}_r$ of length $N_{\rm pix}$, while the scene $\mathbf{s}$ is the concatenation of every source spectrum on its own wavelength grid, of length $N_s\simeq N_{\rm src}N_\lambda$. The data and the scene live in different index spaces. The vectors $\mathbf{d}_r$ and $\mathbf{n}_r$ run over detector pixels, whereas $\mathbf{s}$ runs over source and wavelength, and the $N_{\rm pix}\times N_s$ matrix $\mathbf{A}_r$ maps the scene onto the detector at roll $r$. Because each source disperses along a two-dimensional trace, $\mathbf{A}_r$ mixes both detector axes rather than acting along one. The same scene $\mathbf{s}$ produces all three roll frames. The three equations therefore stack into
\begin{equation}
  \begin{bmatrix}\mathbf{d}_{0^\circ}\\ \mathbf{d}_{45^\circ}\\ \mathbf{d}_{90^\circ}\end{bmatrix}
  =\begin{bmatrix}\mathbf{A}_{0^\circ}\\ \mathbf{A}_{45^\circ}\\ \mathbf{A}_{90^\circ}\end{bmatrix}\mathbf{s}
  +\begin{bmatrix}\mathbf{n}_{0^\circ}\\ \mathbf{n}_{45^\circ}\\ \mathbf{n}_{90^\circ}\end{bmatrix},
  \label{eq:scenestack}
\end{equation}
an over-determined system of $3N_{\rm pix}$ equations for the single $N_s$-vector $\mathbf{s}$. Recovering that one shared scene from three distinct projections $\mathbf{A}_r$ is what makes the crowded field separable.

The final spectra are recovered by a regularized joint fit over all orientations with covariance and contamination flags propagated into the redshift catalog. Objects that remain degenerate after the three-angle fit are not forced into the cosmology sample. They are flagged as low-confidence redshifts or excluded by the survey mask.
