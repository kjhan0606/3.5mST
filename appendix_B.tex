...
The high-contrast performance discussed in Section~B.3 poses a fundamental
...
a characteristic planet-to-star contrast of order $10^{-10}$, approximately
one order of magnitude deeper than the $\sim10^{-9}$ post-processing
...
The nominal inner working angle (IWA) introduces an additional limitation.
The IWA should not be interpreted as a sharp boundary beyond which the full
limiting contrast is immediately achieved. Residual stellar light remains
significant immediately outside the IWA, while both achievable contrast and
coronagraphic throughput improve progressively with angular separation.
...
The contrast limit discussed in Section~\ref{sec:earthlike_multi_epoch} does not apply equally to Earth-like planets around all main-sequence stars. In reflected light, the planet-to-star flux ratio can be written as
...
Earth-equivalent insolation distance (EEID) scales approximately as
...
moves closer to the host star. For a planet with the same radius, albedo, and
illumination phase, the smaller orbital separation increases the
...
strategy. For a circular orbit, the characteristic orbital period at the
Earth-equivalent insolation distance can be estimated from
...
separations are sampled. A cadence of order $P/6$--$P/4$ provides a useful
starting point for such a search, although the optimum cadence must ultimately
