...
The enabling technology is the coronagraph. A coronagraph blocks or
...
shows the point-source contrast against angular separation. The raw
coronagraph reaches a floor near $10^{-8}$ beyond the inner working angle,
and reference- and angular-differential post-processing lower it toward
$10^{-9}$. A Jupiter analog at $4\times10^{-9}$ sits above that floor, and
an Earth analog at $2\times10^{-10}$ sits below it. The floor is set by
wavefront stability. A wavefront error of root-mean-square $\sigma$
...
$(2\pi\sigma/\lambda)^2/N$. Figure~\ref{fig:contrast} (right) inverts this
relation. A $10^{-9}$ floor then demands $\sigma$ of order 100\,pm, and an
Earth analog a few tens of picometres. A wavefront-error spectrum
concentrated near the inner working angle tightens the requirement
further. This tens-of-picometre regime, rather than the collecting area,
is what separates the giant-planet program from the Earth-analog program
\cite{bailey23}.
...
and the raw-contrast floor versus wavefront-error stability (right) from
the $(2\pi\sigma/\lambda)^2$ scatter relation divided across the
$\sim$$10^{3}$ controllable modes of the dark hole. The design floor of
$10^{-9}$ requires a wavefront stable at the $\sim$100\,pm level, and an
Earth analog a few tens of picometres, matched to the Roman Coronagraph
technology path \cite{bailey23}.}
\label{fig:contrast}
\end{figure}
...
renormalized unit weight error below 1.4 serves as a data-driven flag
for astrometrically clean single stars \cite{lindegren21}. This cut
leaves 39 single FGK dwarfs within 10.3\,pc, 143 within 15.4\,pc, and
7{,}411 within 53.5\,pc (Table~\ref{tab:targets}). Wide, well-resolved
...
\toprule
Horizon & Enables & All stars$^{a}$ & FGK & Single FGK$^{b}$ & M dwarfs \\
\midrule
...
$^{b}$Astrometrically clean single-star candidates, Gaia RUWE $<1.4$.}
\label{tab:targets}
\end{table}
...
ESO/S.~Brunier). Points are astrometrically clean single FGK dwarfs from
Gaia DR3 \cite{gaiadr3}. Orange marks those within the 15.4\,pc
Earth-analog horizon and blue those from 15.4 to 53.5\,pc. Named nearby
...
high-latitude sky. Magenta rings mark the 13 stars whose habitable zone
is resolvable at $3\lambda/D$ (Table~\ref{tab:hztargets}).}
\label{fig:mollweide}
\end{figure}
...
single FGK targets number 143 within 15.4\,pc and 7{,}411 within
...
reflected-light survey addresses of order a few dozen single FGK stars
within the 10--15\,pc horizon. The giant-planet survey at $10^{-9}$
contrast opens several thousand single FGK stars within 53.5\,pc, where
...
filaments, while the FGK targets fill the clean high-latitude sky. The
...
$\alpha$~Centauri near the plane at $b=-0.7^\circ$. Magenta rings mark the
13 habitable-zone-resolvable targets of Table~\ref{tab:hztargets}.}
\label{fig:cirrus}
\end{figure}
...
\subsection{Discovery space and expected yield}
\label{sec:yield}
...
\caption{Single-visit completeness for a star at 10\,pc in the
...
are marked. The reachable region is giants and large sub-Neptunes at
$1$--$10$\,AU.}
\label{fig:completeness}
\end{figure}

Summing the completeness over the 7411 astrometrically clean single FGK
dwarfs within 53.5\,pc, weighted by measured occurrence rates, gives the
expected detections. Cold giants occur around roughly 15\% of FGK stars
\cite{fernandes19,fulton21}, and temperate small planets are more common
still \cite{fulton17,bryson21}. Figure~\ref{fig:yieldbar} shows the
result. At the $10^{-9}$ floor of current technology the mission images of
order 140 cold giants and almost no smaller planets. Deepening the floor
to $10^{-10}$ raises the giant yield above 700 and opens roughly 60
temperate sub-Neptunes and a handful of rocky habitable-zone planets. The
giant-planet census is therefore in reach now, and the terrestrial census
is the reward of the contrast roadmap of Section~\ref{sec:roadmap}.

\begin{figure}[tp]
\centering
\includegraphics[width=0.74\textwidth]{yield_bar.png}
\caption{Expected reflected-light detections over the 7411 single-FGK
sample within 53.5\,pc, by planet class and contrast floor. Occurrence
rates: cold giants 0.15 \cite{fernandes19,fulton21}, temperate
sub-Neptunes 0.30 \cite{fulton17}, rocky habitable-zone 0.30
\cite{bryson21}. The current $10^{-9}$ floor delivers the giant census;
the $10^{-10}$ floor delivers the terrestrial one.}
\label{fig:yieldbar}
\end{figure}
...
$R=5000$ resolving power also enables cross-correlation of a faint
planetary spectrum against molecular templates. The planet's orbital
motion Doppler-shifts its lines away from the residual starlight, which
recovers detections at contrast levels above the raw coronagraphic
floor. These measurements complement transit spectroscopy, which probes
...
planet holds far less than one photon. A band is therefore measured by
cross-correlating its whole forest of lines, which combines them and
raises the significance as the square root of their number.
...
\cite{leinert98}, and the exozodiacal light \cite{ertel20}. These times
are photon-noise-limited lower bounds. The model assumes the speckle field
subtracts to its own photon noise and omits detector read noise and
residual-speckle systematics, both of which lengthen a real integration.
Reflected-light giant planets, at contrasts of $10^{-9}$ to $10^{-8}$
\cite{traub10}, reach a molecular detection in minutes to a few hours
around the nearest bright FGK hosts. An Earth twin at $2\times10^{-10}$
contrast requires hundreds of hours per band on a 3.5\,m aperture. That
places true Earth-analog spectroscopy in the domain of the larger
Habitable Worlds Observatory \cite{seager25}. The $R=5000$ program
therefore characterizes the atmospheres of nearby giants and warm
sub-Neptunes, and it measures the reference reflected-light spectra
against which the flagship missions will interpret their terrestrial
detections.

\begin{figure}[tp]
\centering
\includegraphics[width=0.78\textwidth]{exo_spec_etc.png}
\caption{Exposure time for a $5\sigma$ cross-correlation detection of a
...
\cite{ertel20}, with a band of 500 lines of mean depth 0.2. Reflected-light giants are reached in hours
around the nearest bright hosts. Earth twins fall beyond a practical
single-target budget for this aperture.}
\label{fig:specetc}
\end{figure}
...
Table~\ref{tab:hztargets} applies this model to the 13 catalog stars of
Section~\ref{sec:catalog} whose habitable zone clears the inner working
angle. The nearest cool dwarfs, 61~Cygni~A and $\epsilon$~Indi, reach an
Earth-analog molecular band in about ten hours, because their compact
habitable zones raise the reflected-light contrast. The more luminous
hosts have wider and fainter habitable zones and require hundreds of
hours. These 13 stars are ringed in Figures~\ref{fig:mollweide}
and~\ref{fig:cirrus}.
...
angles at 550\,nm and clear the $10^{-9}$ contrast floor. These six are
the coronagraph's first-light targets, planets whose existence, orbit, and
mass are already known. A detection is therefore a characterization rather
than a search. The five widest known planets, including the directly
...
The exposure-time model of Section~\ref{sec:specetc} turns the target list
into a mission timeline. Figure~\ref{fig:ops-exo} shows the cumulative observing
time when the targets are taken cheapest first. Spectroscopy of a Jupiter
analog around each of the 143 nearest single FGK stars takes a few hours
per bright target, and 122 of the 143 are reached within one mission year
of observing. The 13 habitable-zone Earth-analog measurements are far more
expensive and fill a comparable amount of time on their own. A three-year
program therefore completes the giant-planet spectroscopic census of the
solar neighborhood and makes a first attempt on the nearest habitable
zones.

\begin{figure}[tp]
\centering
\includegraphics[width=0.76\textwidth]{ops_timebudget.png}
\caption{Cumulative number of targets whose atmosphere reaches a $5\sigma$
molecular detection, against observing time, ordering targets cheapest
first, from the $R=5000$ model of Section~\ref{sec:specetc}. Giant-planet
spectroscopy of the 143 nearest single FGK stars ($C=4\times10^{-9}$)
largely completes within one observing year; the 13 habitable-zone
Earth-analog measurements are much slower.}
\label{fig:ops-exo}
\end{figure}
...
The roadmap groups the science by technical difficulty. Reflected-light
imaging of Jupiter analogs at $10^{-9}$ contrast is the core program
that current coronagraph technology supports \cite{galicher23}.
Spectroscopic follow-up of the brightest detections with the $R=5000$
spectrographs follows as contrast control matures. Earth-analog work at
$10^{-10}$ contrast defines the long-term goal, and it positions the
mission as a technology and target-selection precursor for the dedicated
flagship observatories \cite{quanz22,seager25}. Each stage delivers
standalone science. Together they convert the solar neighborhood, the
single point marked on the Galactic map of Figure~\ref{fig:mwhorizon},
into a surveyed volume of characterized planetary systems.
...
The second is that the cadence differs by planet type. A Jupiter analog
reaches its image and its $R=5000$ spectrum within a single window, so the
giant-planet survey proceeds target by target and front-loads into the first
three years. An Earth analog in the habitable zone should instead be recovered
near maximum elongation, which recurs only about once a year, and each
detection needs hundreds of hours. The habitable-zone campaign is therefore
revisited across several years.
...
month-by-month field of regard. As in the companion cosmology survey plan,
the year ranges are an existence proof rather than a committed schedule. The
quoted science time sums to about $13{,}000$\,hr, near 30\% of the wall-clock
available over five years, which matches the 30\% observing efficiency (about
$2630$ observing hours per year) adopted for the exposure budget of
Section~\ref{sec:specetc}. That budget absorbs the Sun-avoidance field of
regard, the dark-sky preference, and the slew and calibration overhead of a
robotic coronagraph. A three-year effort would then reach the
giant-planet spectroscopic census of the solar neighborhood, with the
remaining time given to the habitable-zone campaign and to the repeat epochs
that turn the imaged giants into dynamical masses.
...
\textbf{Mission phase} & \textbf{Nominal period} & \textbf{Science time} & \textbf{Main deliverable}\\
 & mission year & hr & \\
\midrule
Commissioning and wavefront ramp & Year 1 & 300--500 hr & Dark-hole control to the $10^{-9}$ floor, with point-spread-function and contrast calibration.\\
...
First-light known giants & Years 1--2 & 300--500 hr & Reflected-light recovery of the six known planets that clear the working angles (Table~\ref{tab:firstlight}).\\
Reflected-light giant survey (143 FGK) & Years 1--3 & 3800--5200 hr & Images and $R=5000$ spectra of Jupiter analogs, one target per observability window.\\
Giant orbit determination & Years 2--5 & 600--1000 hr & Repeat epochs across the mission for orbital arcs and dynamical masses.\\
HZ Earth-analog campaign (13) & Years 2--5 & 5200--8000 hr & Deep, multi-epoch reflected-light integrations of the 13 resolvable habitable zones (Table~\ref{tab:hztargets}), near maximum elongation.\\
...
\bottomrule
\end{tabularx}
\caption{Illustrative five-year coronagraph observing program. The
science-time ranges follow from the field-of-view, contrast-floor, and
overhead assumptions of Sections~\ref{sec:horizon} and~\ref{sec:specetc}, and
they sum to about $13{,}000$\,hr, near 30\% of the five-year wall-clock,
matching the 30\% observing efficiency of the exposure budget. The
Jupiter-type giant programs proceed target by target within single
observability windows and front-load into the early years. The habitable-zone
campaign spans several years because an Earth analog is recoverable only near
its yearly maximum elongation.}
...
\caption{Five-year coronagraph observing timetable. \emph{Top}: the observing
programs of Table~\ref{tab:exotimeline} across the mission years, colored by
planet type, with the cell shade tracking the scheduled science hours per
month. The Jupiter-type giant programs front-load into Years~1--3, each target
reached within a single $\sim$1-year observability window, while the
Earth-type habitable-zone campaign spans Years~2--5 because each target is
revisited near maximum elongation across several years. \emph{Bottom}: the
fraction of the 143 giant-survey stars and of the 13 habitable-zone targets
inside the $54$--$126^\circ$ solar-elongation field of regard on the fifteenth
of each month, from an \texttt{astropy} Sun-position calculation. About half
the giant sample is accessible in any month, while the smaller habitable-zone
sample shows a stronger seasonal variation set by its sky distribution. The
field of regard, the dark-sky preference of high ecliptic latitude, and the
slew and calibration overhead together set the observing efficiency near
30\% of wall-clock, about $2630$ observing hours per year. The year ranges
are an existence proof, not a committed
schedule.}
\label{fig:exotimeline}
\end{figure}

\section{Target Catalog: Single FGK Stars within 15.4\,pc}
\label{sec:catalog}

Table~\ref{tab:catalog} lists the 143 astrometrically clean single FGK
dwarfs inside the 15.4\,pc Earth-analog horizon of Section~\ref{sec:horizon}.
These are the prime reflected-light targets mapped in
...
with the quantities that plan a reflected-light observation. The
Earth-insolation distance $a_{\rm HZ}=\sqrt{L/L_\odot}$\,AU marks the
habitable zone, and its angular separation
$\theta_{\rm HZ}=a_{\rm HZ}/d$ sets whether the zone is resolvable. The
final column of each block flags the stars whose habitable zone clears the
$3\lambda/D$ inner working angle at 550\,nm. Only 13 of the 143 stars meet
that condition. The remainder are limited to their wider giant planets, in
line with the horizon arithmetic of Section~\ref{sec:horizon}. Twenty-two
