...
\caption{Full numerical exposure-time calculation for the slitless survey. \emph{Top}: the modelled sky as an AB surface brightness, the zodiacal light (observed solar spectrum scattered and reddened per Leinert et~al.\ \cite{leinert1998}, plus interplanetary-dust thermal emission) together with the telescope thermal self-emission that rises beyond $2\,\mu$m. \emph{Bottom}: the resulting $5\sigma$ line-flux depth $F_{5\sigma}(\lambda)$ for three exposures, deepest near $1.6\,\mu$m where the sky is darkest, with the conservative $10^{-16}$ planning anchor of Eq.~\eqref{eq:f5sigma} marked for reference.}
\label{fig:etc}
\end{figure}
...
\midrule
0.6 & 28.67 & 27.94 & -- & --\\
1.0 & 27.99 & 27.40 & 25.32 & 29.67\\
1.5 & 27.99 & 27.24 & 25.18 & 29.29\\
2.0 & 26.86 & 26.01 & -- & 29.00\\
2.5 & 24.33 & -- & -- & 28.75\\
3.5 & -- & -- & -- & 28.43\\
\bottomrule
\end{tabularx}
\caption{Imaging $5\sigma$ point-source limiting AB magnitude in a $1$\,hr exposure through a $0.3\,\mu$m band, for the four telescopes evaluated with the same calculator from their real apertures and detector parameters. Dashes mark wavelengths outside a mission's band. JWST is deepest (largest, cold aperture), followed by the $3.5$\,m concept, Roman, and Euclid. The $3.5$\,m entry at $2.5\,\mu$m is depressed by warm-optics thermal background, which cooling removes.}
...
\caption{Proposed ELG survey fields on an equatorial Mollweide projection. The north and south Galactic caps at $|b|\gtrsim45^\circ$ (green and blue) avoid the Milky Way zone ($|b|<20^\circ$, red) and lie within the DESI, Euclid, Rubin/LSST, and Roman footprints for cross-survey synergy. Gold stars mark the legacy HST and Subaru deep fields that fall within the caps (Figure~\ref{fig:deepzoom}).}
\label{fig:skymap}
\end{figure}
