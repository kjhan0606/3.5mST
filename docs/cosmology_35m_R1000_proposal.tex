...
For the faintest diffuse work the dedicated deep-imaging block is sited on the cleanest sky available, found by minimising the mean Planck 857\,GHz intensity over a footprint-sized aperture across all $|b|>48^\circ$ sky \cite{planck2018hfi}. The minima fall at $(\alpha,\delta)=(210^\circ,+38^\circ)$, $b=+72^\circ$, in the north and $(344^\circ,-48^\circ)$, $b=-59^\circ$, in the south, both with mean $857$\,GHz intensities of $0.95$\,MJy\,sr$^{-1}$, about a quarter of the all-sky median of $3.6$\,MJy\,sr$^{-1}$. The northern minimum sits essentially on the flagship Bo\"otes anchor, whose window averages $1.01$\,MJy\,sr$^{-1}$, while the southern imaging field improves on the SXDS window ($1.30$\,MJy\,sr$^{-1}$) by about $27\%$. Figure~\ref{fig:imgfields} zooms on both fields and confirms directly on the dust map that the footprints are free of cirrus structure, and their locations are marked on the all-sky view of Figure~\ref{fig:skymap}. The imaging fields are observed in the five survey bands, $g$ ($0.5\,\mu$m) and $z$ ($0.9\,\mu$m) on the CCD arm and $J$, $H$, and $K$ ($1.2$, $1.6$, $2.2\,\mu$m) on the HgCdTe arm (Figure~\ref{fig:compareimg}). Because the dichroic feeds both arms simultaneously, each pointing collects one optical and one near-infrared band at once, so the full five-band set costs three passes of the footprint. The deep $g$ and $H$ pair carries the diffuse-light science, with the surface-brightness reach of Table~\ref{tab:lsbsurvey}, while $z$, $J$, and $K$ at intermediate depth supply the colours, stellar-population diagnostics, and photometric redshifts of the LSBG and UDG catalog.

\begin{figure}[tp]
\centering
\includegraphics[width=\textwidth]{imaging_fields_zoom.png}
\caption{The dedicated low-cirrus imaging-survey fields, one per Galactic cap, on the Planck 857\,GHz thermal-dust map \cite{planck2018hfi}, in the same gnomonic (TAN) projection with the RA/Dec graticule as Figure~\ref{fig:deepzoom}. Each field is the minimum of the mean $857$\,GHz intensity over a footprint-sized aperture searched across all $|b|>48^\circ$ sky. The $12.5^\circ$ footprint is paved with its $30'\times30'$ survey tiles (cyan) and the measured mean intensity is annotated. In the north the imaging field partially overlaps the ELG wide tier on the Bo\"otes anchor (orange outline), since the flagship anchor already sits next to the cirrus minimum of the northern cap. Both footprints sit in dark holes of the cirrus at a quarter of the all-sky median intensity, verifying on the data itself that the deep diffuse-light imaging is placed on the cleanest available sky.}
\label{fig:imgfields}
\end{figure}
