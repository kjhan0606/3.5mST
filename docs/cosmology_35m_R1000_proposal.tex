...
Type Ia supernovae (SNe Ia) are the most mature distance indicator in cosmology and the only one of the companion programs that interacts directly with the same dark-energy science driving the flagship ELG survey. $R=1000$ slitless spectroscopy is well matched to the broad spectral features of SNe Ia. The analysis can bin to $R\simeq300$--500 for classification, phase determination, dust and color estimation, and population diagnostics while retaining ample signal. The time-domain program should use repeated fields, not isolated single-object follow-up. The \fovsmall\ field is acceptable for a deep cadence field. The \fovlarge\ field enables multiple active supernovae per pointing and improves host-galaxy template efficiency. Beyond classification and host-redshift control, the mission has an additional capability. At the redshifts where it operates, its $0.36$--$3.0\,\mu$m band captures the \emph{rest-frame near-ultraviolet} of SNe Ia, where the dominant intrinsic-luminosity systematic of the distance ladder is encoded.
...
\subsection{Lyman-\texorpdfstring{$\alpha$}{alpha} Forest and Absorption Systems}
...
``Cosmological parameter estimation using Monte Carlo Markov chains,''
