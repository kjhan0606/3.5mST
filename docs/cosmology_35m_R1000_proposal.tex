...
\begin{tikzpicture}[font=\sffamily\footnotesize]
  \fill[paper] (-0.35,-0.35) rectangle (16.0,6.55);
  \node[anchor=west,font=\bfseries\large,color=ink] at (0,6.18) {Crowded-field slitless spectroscopy is solved as a multi-angle scene model};

  \foreach \x/\ang/\lab in {0.15/0/{0$^\circ$},4.25/45/{45$^\circ$},8.35/90/{90$^\circ$}} {
    \begin{scope}[xshift=\x cm,yshift=2.35cm]
      \fill[white,draw=line,rounded corners=2pt] (0,0) rectangle (3.45,2.85);
      \clip (0,0) rectangle (3.45,2.85);
      \node[anchor=west,color=muted] at (0.18,2.58) {orientation \lab};

      \foreach \px/\py/\col/\len in {
        0.48/2.05/teal/1.62,
        0.72/1.45/gold/1.75,
        1.45/1.45/blue/1.72,
        1.08/0.82/green/1.55,
        2.02/1.02/rose/1.48} {
        \begin{scope}[rotate around={\ang:(\px,\py)}]
          \draw[\col!35,line width=5.2pt,opacity=0.70,line cap=round] (\px+0.14,\py) -- ++(\len,0);
          \draw[\col,line width=1.05pt,opacity=0.95] (\px+0.14,\py) -- ++(\len,0);
          \draw[white,line width=0.75pt,opacity=0.75] (\px+0.54,\py-0.11) -- ++(0,0.22);
          \draw[white,line width=0.75pt,opacity=0.75] (\px+0.98,\py-0.11) -- ++(0,0.22);
          \draw[white,line width=0.75pt,opacity=0.75] (\px+1.32,\py-0.11) -- ++(0,0.22);
        \end{scope}
        \fill[\col,draw=white,line width=0.25pt] (\px,\py) circle (2.4pt);
      }

      \begin{scope}[rotate around={\ang:(1.45,1.45)}]
        \fill[rose!78,opacity=0.34,rounded corners=0.5pt] (1.58,1.29) rectangle (2.48,1.61);
        \draw[rose,line width=0.75pt,dashed] (1.58,1.29) rectangle (2.48,1.61);
        \node[anchor=south,color=rose,font=\scriptsize] at (2.03,1.64) {overlap};
      \end{scope}

      \draw[-{Stealth[length=2.0mm]},color=muted] (0.28,0.28) -- ++({cos(\ang)*0.78},{sin(\ang)*0.78});
      \node[anchor=west,color=muted] at (1.1,0.27) {dispersion};
    \end{scope}
  }

  \draw[-{Stealth[length=2.8mm]},line width=0.9pt,color=muted] (11.95,3.75) -- (12.85,3.75);
  \begin{scope}[xshift=13.0cm,yshift=2.35cm]
    \fill[white,draw=teal,rounded corners=2pt,line width=0.75pt] (0,0) rectangle (2.65,2.85);
    \node[anchor=west,font=\bfseries,color=teal] at (0.18,2.50) {joint solver};
    \node[anchor=west,text width=2.15cm,color=softink] at (0.22,1.86) {forward model all objects, all rolls, all wavelengths};
    \draw[blue,line width=1.1pt] (0.35,0.62) -- (0.85,1.05) -- (1.35,0.78) -- (1.85,1.35) -- (2.28,1.08);
    \draw[gold,line width=1.1pt,dashed] (0.35,0.40) -- (0.85,0.72) -- (1.35,0.54) -- (1.85,0.95) -- (2.28,0.82);
  \end{scope}

  \node[anchor=west,text width=14.8cm,color=muted] at (0.15,0.72)
  {In slitless data, each source is dispersed into a one-sided trace on the detector. The red boxes mark detector pixels where two elongated spectra overlap, not a circular sky region. Rotating by $45^\circ$ and $90^\circ$ changes the trace geometry, allowing the extraction pipeline to assign flux probabilistically and flag irreducible blends before cosmological analysis.};
\end{tikzpicture}
\caption{Self-generated crowded-field slitless observing schematic, following the basic geometry of wide-field slitless spectroscopy. Object images become dispersed traces, and contamination occurs where traces from different galaxies overlap on the detector. The proposal assumes multi-angle observations and a joint scene-reconstruction algorithm to correct spectral overlap.}
\label{fig:crowded}
\end{figure}
...
\begin{tikzpicture}[font=\sffamily\footnotesize]
  ...
  \foreach \y/\survey/\smallp/\smallv/\largep/\largev/\col in {
    ...
    \node[anchor=west,font=\bfseries,color=gold!65!black] at (7.65,\y-0.35) {$9\times$ fewer pointings};
...
\bottomrule
\end{tabularx}
\caption{ETC cross-check against documented JWST sensitivities (STScI JDox) in imaging (NIRCam) and spectroscopy (NIRSpec). The reference column is the JWST-quoted signal-to-noise for the listed source, exposure, and configuration, and the last column is this calculator run on the same $6.5$\,m aperture, detector parameters, and target. Agreement is $\sim10$--$25\%$ in both modes.}
\label{tab:jwstval}
\end{table}
