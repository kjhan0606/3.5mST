...
\caption{Proposed ELG survey fields on an equatorial Mollweide projection. The background is the real COBE/DIRBE 2.2\,$\mu$m near-infrared all-sky map \cite{hauser1998} (CADE HEALPix rendering through the CDS \textit{hips2fits} service) with its intensity scale below, so the near-infrared foreground that the field selection must avoid, the Galactic plane, the bulge, and the bright-star population, is shown directly rather than schematically. The north and south Galactic caps at $|b|\gtrsim45^\circ$ (green and blue outlines) avoid the Milky Way zone ($|b|<20^\circ$, red curves) and lie within the DESI, Euclid, Rubin/LSST, and Roman footprints for cross-survey synergy. Gold stars mark the legacy HST and Subaru deep fields that fall within the caps (Figure~\ref{fig:deepzoom}).}
\label{fig:skymap}
\end{figure}
...
Depth ceiling & Ground limits $\mu\sim28$--$29$, Dragonfly $\mu\sim30$--$31$ mag\,arcsec$^{-2}$ \cite{abraham2014dragonfly} & ETC reach $\mu_{5\sigma}(10'')=29.7$--$31.9$ for $1$--$64$ hr (Figure~\ref{fig:iclimg})\\
\bottomrule
\end{tabularx}
...
\emph{Astrophysical Journal}, 508, 25, 1998.
\url{https://ui.adsabs.harvard.edu/abs/1998ApJ...508...25H/abstract}
