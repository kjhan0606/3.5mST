...
\caption{Rest-frame UV--optical spectra of the optical twins SN\,2011fe (black) and SN\,2011by (blue). \emph{Top:} the two are nearly identical at optical wavelengths ($\lambda>3500$\,\AA) but diverge strongly in the near-ultraviolet. \emph{Bottom:} the flux ratio (11fe/11by, black) compared with synthetic predictions for different progenitor-metallicity ratios $\zeta_1/\zeta_2$ \cite{lentz2000}. The NUV amplitude tracks the metallicity contrast. The optical twin condition removes the light-curve-shape and color degrees of freedom, isolating an intrinsic-luminosity difference that is visible \emph{only} in the rest-frame NUV. Reproduced after Graham et~al.\ \cite{graham2015}.}
\label{fig:uvtwins}
\end{figure}
