...
Figure~\ref{fig:voidwedge} makes this target selection concrete with real data rather than an assumed catalog. It shows the actual SDSS-I/II Main Galaxy Sample spectroscopy in the same declination range as the imaging-only wide-extension field beside the Lockman Hole (Section~\ref{sec:lsb}), projected as a comoving-distance wedge. Five voids from the independent Bayesian catalog of Malandrino et al.\ \cite{malandrino2026} fall inside this one field, each with a published position, redshift, and radius rather than an estimate made for this proposal. The same cross-match against published void catalogs, repeated across the wide, medium, and deep spectroscopic tiers, is what selects the void interiors that this section's search tiles.

\begin{figure}[tp]
\centering
\includegraphics[width=\textwidth]{void_wedge_demo.png}
\caption{Cosmic-web wedge diagram spanning the same declination range, $43^\circ$--$61^\circ$, as the Lockman Hole imaging-only wide-extension field (Section~\ref{sec:lsb}), split into four equal Dec bands shown side by side. Grey points are real SDSS-I/II Main Galaxy Sample spectroscopic redshifts (\texttt{survey='sdss'}, excluding BOSS/eBOSS), queried live from SkyServer DR17 over $0<z<0.3$. Orange circles mark the five voids of the independent Bayesian catalog of Malandrino et al.\ \cite{malandrino2026} that fall inside this field, plotted at their own cataloged position and radius. Red lines mark the field's RA boundary, $147.4^\circ$--$176.6^\circ$, with the field center dashed. This is a target-selection demonstration on one real field, not a claim that this particular imaging-only field itself carries the spectroscopy; the void search of this section runs on the wide, medium, and deep spectroscopic tiers.}
\label{fig:voidwedge}
\end{figure}
