...
\hypersetup{
  ...
  citecolor=teal,
  urlcolor=gold,
  pdftitle={R=1000 Cosmology Proposal for a 3.5-meter Segmented-Mirror Robotic Space Telescope},
...
\fancyhead[R]{\sffamily\footnotesize R=1000 Cosmology Proposal}
...
\vspace*{0.35cm}
{\sffamily\color{white}\fontsize{25}{30}\selectfont\bfseries Cosmology with Native $R=1000$ Slitless Spectroscopy}\\[1.2mm]
{\sffamily\color{white}\fontsize{16}{20}\selectfont with the \facility}\\[3mm]
{\sffamily\color{white!78}\large A one-column science proposal for a wide-field slitless survey}\\[10mm]

\begin{leadbox}[title=Executive Summary]{teal}
\textbf{The \facility\ is proposed as a robotic wide-field cosmology mission with native $R=1000$ slitless spectroscopy.}
The central science case is a dense emission-line galaxy redshift survey for BAO and redshift-space distortions, supported by supernova and quasar programs that exploit the stability, multiplexing, and repeatability of space operations. The design assumes a crowded-field slitless environment from the start. Every science tile is observed in three orientations, approximately $0^\circ$, $45^\circ$, and $90^\circ$, so that spectral overlap can be solved by a joint scene-reconstruction algorithm. The flagship survey covers 100--300 \degti, requires the \fovlarge\ field for efficiency, and is complemented by a deep pencil-beam tier and a supernova time-domain tier.
\end{leadbox}
...
\caption{Two-dimensional galaxy correlation function from the Horizon Run 4 simulation, shown in real space (left) and redshift space (right) \cite{hr4}. The red circle marks the BAO location. The nearly circular real-space BAO ring provides the standard ruler. Its anisotropic appearance in redshift space contains the radial BAO scale and RSD growth information.}
\label{fig:hr4xi}
\end{figure}
...
Table~\ref{tab:triage} collects the program-level scope implied by the preceding sections. It separates the core ELG BAO/RSD survey from companion programs whose requirements are compatible with the same slitless architecture but should not drive the flagship performance claims.
