...
\caption{The dedicated low-cirrus imaging-survey fields, one per Galactic cap, on the Planck 857\,GHz thermal-dust map \cite{planck2018hfi}. Each field is the minimum of the mean $857$\,GHz intensity over a footprint-sized aperture searched across all $|b|>48^\circ$ sky, and each panel is a gnomonic zoom with the $12.5^\circ$ wide-tier tiling boundary overlaid (orange, $30'$ grid drawn faintly) and the measured mean intensity annotated. Both footprints sit in dark holes of the cirrus at a quarter of the all-sky median intensity, verifying on the data itself that the deep diffuse-light imaging is placed on the cleanest available sky.}
\label{fig:imgfields}
\end{figure}
