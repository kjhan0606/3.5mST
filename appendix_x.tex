...
accretion-disk profiles, velocity structure, and precision line ratios. The
native $R\simeq5000$ data may be binned when broad features do not require
that resolution. The instrument study must preserve calibrated throughput to
...
testable program must specify the observable and observing interval for each
source class. Three experiments provide that specification. Rapid observations
of merger counterparts measure spectral evolution over hours to days.
Repeated spectra of accreting binaries measure line profiles as functions of
orbital phase and accretion state. Uninterrupted imaging of white dwarfs
measures pulsation frequencies while spectroscopy determines the atmospheric
...
unrelated field source may overlap the trace. Direct imaging, a model of every
trace in the field, and more than one dispersion orientation remain necessary
for the measurement.
...
The two resolving-power settings provide different measurements for different
source classes. Cataclysmic-variable disk lines often span several hundred to
more than one thousand $\kms$. The line wings and double peaks are consequently
sampled by several resolution elements in the selectable $R\simeq5000$ bands.
Kilonova ejecta instead
...
The spectra provide more than a redshift or a classification label. Their
principal information is the joint time evolution of the continuum and line
profiles. The spectral fit must report the quantities in
Table~\ref{tab:observables} together with their covariance matrix.
...
alert channel records short-lived kilonovae and accretion episodes. Blind
extraction searches the dispersed data for compact emission-line sources that
are absent from existing catalogs.

Catalog classifications, optical or X-ray variability, orbital information,
and predicted spectral brightness define the initial selection. The final
selection follows after the ETC includes measured throughput, zodiacal
...
KMTNet transient observations can provide an external trigger for the
compact-object program. Each notice supplies a discovery time, a sky position,
and an optical light curve that rank the target before the spacecraft begins a
spectroscopic visit. The notice does
...
The scene model must be tested with a transient that brightens or fades during the
...
$\iota$. A radiative-transfer model maps these observables to the masses,
...
sequence. The minimum mission-level requirement is not merely a fast slew. Before the
first exposure, the observatory must validate the alert, calculate visibility,
update the queue, acquire the target, assess data quality, and disseminate the
result. A response time that is fast at the
...
spectrograph. They are unresolved, recurrently variable, and rich in emission
and absorption lines. The standard disk-instability model attributes dwarf-nova outbursts to a
...
observed transition demonstrates that a time series of spectra follows
...
At $R=5000$ the spectrum separates the broad disk-line structure while
retaining sufficient continuum sensitivity for the reference survey. The nominal velocity scale is
...
neighboring field objects across the full sample. A nearby contaminating star may
...
external high-energy information. The program therefore coordinates the
...
The numerical kernel remains in \texttt{slitless\_etc.py}. Distinct observing
modes separate imaging, isolated-line spectroscopy, template spectroscopy,
and validation. The compact-object calculation invokes only these spacecraft
modes. A separate cooled mid-infrared calculator remains available for
external ground-facility comparisons and does not describe a 3.5-m spacecraft
instrument. The mode separation prevents an AB magnitude, an integrated line
flux, and a calibrated $F_\lambda$ spectrum from entering the same calculation.
...
flight performance and will be revised after component transmission, detector
...
The compact-object program depends on external facilities for discovery,
high-energy measurements, and the final physical inference.
...
imaging supplies the pulsation frequencies. Stellar models infer the interior
...
from the observing database. It then assigns every program to a common
260-week capacity model. The current reference solution reserves 618.3 hr for
...
brightness. The figure therefore demonstrates available mission capacity. It
does not predict the number of gravitational-wave counterparts or accretion
transients that will satisfy the trigger criteria.
...
The compact-object program becomes a mission-level performance claim only
after the following validation products meet quantitative acceptance tests.
...
recovery of peak separation, wings, velocity modulation, and transitions between states
...
photometric noise. The class-specific demonstrations convert the proposed
...
slitless spectrograph with $R\simeq1000$ as the broad-survey setting and
selectable $R\simeq5000$ bands for precision line profiles. Calibrated
throughput to $2.70\micron$ and an operational band-edge goal of
$3.0\micron$ extend the common instrument study. The point-source nature of
compact objects preserves the native resolution for an isolated trace. Direct
images, multiple orientations, and a scene model are still required to control
overlap. The resulting spectra measure velocities, ionization, outflows, disk
kinematics, transitions between states, and transient spectral evolution when
the ETC establishes the required S/N.
