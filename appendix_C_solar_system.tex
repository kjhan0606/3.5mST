...
and its force model. The final comparison is made against a separately
...
\caption{Actual OGFinder DS9 runtime capture using five calibrated DECam
exposures from the DECam Asteroid Database DR2. The NEO menu executed the
local moving source detector. Cyan circles show four linked candidate
tracks, with only the matching epoch displayed in each frame. The status
line records successful association. These marks are candidates for visual
review and do not by themselves validate an orbit.}
...
astrometry. Figure~\ref{fig:codes-openorb-calculation} shows the resulting
posterior in $(a,e)$ and $q$ together with the NASA/JPL comparison point.
...
tracklet. Orange points are posterior samples and the star is the separate
NASA/JPL solution for the same numbered object. The broad posterior shows
why a short optical arc must not be reported as a unique orbit.}
...
\bibitem{Granvik2009}
Granvik, M., Virtanen, K., Oszkiewicz, D., \& Muinonen, K. 2009,
``OpenOrb: Open-source asteroid orbit computation software including
statistical ranging,''
\textit{Meteoritics \& Planetary Science}, 44, 1853.
\href{https://doi.org/10.1111/j.1945-5100.2009.tb01194.x}
{doi:10.1111/j.1945-5100.2009.tb01194.x}.

\bibitem{DataLabDAD}
NOIRLab Astro Data Lab,
``DECam Asteroid Database DR2 and Simple Image Access service.''
...
\url{https://www.nasa.gov/solar-system/new-report-assesses-status-of-detecting-near-earth-asteroids/}.
