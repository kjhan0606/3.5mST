...
smaller copy of a ground survey. Its case rests on access to a different
observational phase space: low-background thermal infrared imaging, stable
calibration without an atmosphere, a larger usable time window, and a field
of regard that can be engineered closer to the Sun.
...
$X\simeq\sec z$ away from the horizon, and atmospheric extinction contributes
...
This is the practical sense in which the ground-based sky window is smaller:
the excluded region is set by data-quality and scheduling requirements, not
only by whether the target is geometrically above the horizon.
...
reduce instrumental thermal emission enough to exploit this full band.
...
Stable PSF and throughput are achievable, but detector persistence, cosmic
...
survey speed than the largest ground facilities. It is recovery and
characterization from a stable space platform, together with an optional
MIR discovery mode in wavelength and solar-elongation regimes that the
ground cannot survey efficiently.
\end{resultbox}
...
\begin{equation}
  ...
  \label{eq:h-diameter}
\end{equation}
The absolute magnitude $H$ removes observing geometry: it is defined at
$r=\Delta=1$\,au and zero phase angle. For a real observation, the apparent
visual magnitude and the corresponding diameter threshold are
\cite{Horizons}
\begin{equation*}
\begin{aligned}
 m_V &=
...
\end{aligned}
\end{equation*}
where $r$ is the heliocentric distance, $\Delta$ is the telescope--NEO
distance, both in au, and $\Phi(\alpha)$ is the normalized phase function.
...
geometric albedo $p_V$ at telescope--NEO distance $\Delta$. The shared color
scale and continuous white contours give diameter $D$. The reference
calculation sets $r=1$\,au and $\Phi(\alpha)=1$, so the color at a survey's
$m_V=m_{\rm lim}$ is an optimistic minimum detectable diameter. General
geometry scales the result by $r/\sqrt{\Phi(\alpha)}$. This is a flux
threshold relation, not a survey-completeness prediction.}
