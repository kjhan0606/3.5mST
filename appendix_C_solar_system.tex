...
phase curves, recovery, and long-arc orbit improvement. The \facility\ does
not carry a mid-infrared channel. Ground-based MIR telescopes provide the
thermal fluxes needed for diameter and albedo inference. The mission supplies
the contemporaneous reflected-light measurements and observing geometry that
make the joint thermal analysis physically useful.
\end{resultbox}
...
includes stable optical and near-infrared calibration without an atmosphere,
a larger usable time window, and a field of regard that can be engineered
closer to the Sun. The MIR measurement belongs to the coordinated ground
segment rather than the 3.5ST payload.
...
telescope produce a high photon background. Ground observations address the
background with chopping, nodding, and short targeted integrations
\cite{ESOVISIR2019}. The resulting mode does not support a continuous
all-sky MIR census. The mode does support thermal characterization of a
selected target with a sufficiently accurate ephemeris. This targeted role
defines the adopted division of responsibility.
...
The baseline 0.2--1.5\,$\mu$m range. No MIR detector or MIR survey mode is
included\\
\addlinespace
Sky and time window &
...
étendue, long temporal baselines, radar constraints, and MIR thermal flux.
The space mission provides weather-independent optical and near-infrared
follow-up, precise astrometry, and rapid scheduling.
\end{enumerate}

\begin{resultbox}
\textbf{\color{cGold}Mission consequence.}
The \facility\ does not claim MIR discovery or MIR sensitivity. The mission
advantage lies in rapid recovery, stable reflected-light characterization,
and orbit refinement. A ground-based MIR partner supplies the independent
...
body. Ground-based thermal-infrared flux breaks much of the degeneracy and
supports a diameter estimate when the observing geometry and thermal model
are known \cite{Mainzer2011,Mainzer2023}.
...
Surveyor provides a space-based reference for the physical wavelength
leverage \cite{Mainzer2023}. Its detector bands do not define 3.5ST
requirements.
...
belongs to the 3.5ST baseline. The Appendix uses thermal physics only to
define the external data needed for diameter and albedo inference.
\end{warningbox}

\subsection{Ground-based MIR follow-up interface}

The thermal measurement enters the program as an external data product.
...
thermophysical fit then returns diameter, visible albedo, and their
covariance.
...
at every exposure &
Places the optical and thermal measurements in one dynamical geometry\\
Joint physical model &
NEATM or thermophysical likelihood with rotational and calibration
uncertainties &
Returns posterior distributions for $D$, $p_V$, $\eta$, and derived impact
energy\\
\bottomrule
\end{tabularx}
...
phase model and an uncertainty term for changing geometry. The collaboration
...
recomputed when the orbit, thermal model, or calibration changes.
...
blind MIR census. Ground optical surveys and the Minor Planet Center (MPC)
provide alerts and preliminary ephemerides. The 3.5ST scheduler converts the
ephemeris and covariance into short exposures that limit trailing. A visit
should contain at least four astrometric measurements over a time span that
resolves the angular motion. A second visit on another night or at a later
geometry extends the orbital arc. CODES determines the repeat interval from
the predicted covariance rather than imposing one fixed interval on every
target.
...
The number of characterized targets depends on the external alert rate, the
...
will quote a characterized-target yield only after a joint scheduler has
folded a representative ground alert stream through the 3.5ST allocation and
the participating MIR facilities.
...
Quantify the characterized-target yield from an external alert stream, the
3.5ST Target-of-Opportunity allocation, and ground MIR availability.
