...
The baseline 0.2--1.5\,$\mu$m observatory provides rapid-response astrometry,
visible and near-infrared taxonomy, rotation and phase curves, recovery, and
long-arc orbit improvement. The instrument study also evaluates calibrated
throughput to 2.70\,$\mu$m with a 3.0\,$\mu$m operational band-edge goal.
A reduction to 2.5\,$\mu$m remains the formal engineering off-ramp if the
thermal, detector, cooling, mass, power, or cost budgets require it. The
\facility\ does not carry a mid-infrared channel. Ground-based MIR telescopes provide the thermal fluxes needed to
infer diameter and albedo. The mission supplies contemporaneous
reflected-light measurements and observing geometry for the joint thermal
analysis.
\end{resultbox}
...
Recent space-observatory studies have established the value of linking
discovery, rapid follow-up, and physical interpretation across observing
facilities \cite{roy2026C,wevers2026C}. This volume applies that network model
to small bodies. The 3.5ST supplies repeated optical and near-infrared
astrometry, spectra, and light curves. Ground facilities supply targeted
mid-infrared thermal measurements because the 3.5ST baseline does not include
an MIR channel. The resulting data products are orbit covariance, size and
albedo constraints, and physically interpretable warning intervals rather
than a generic transient-alert capability.
...
The shared mission scheduler assigns the Appendix C program within the same
...
\begin{table}[H]
\centering
\caption{Database-driven five-year reference allocation for Appendix C. The
...
\midrule
\textbf{Appendix C total} & \textbf{1--5} & \textbf{3057.6} &
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{appendix_C_five_year_timeline.png}
\caption{Weekly Appendix C allocation generated from the shared observing
database. Color gives the fraction of one 168-hr week assigned to each
program. The color scale uses the largest Appendix C allocation and gray
