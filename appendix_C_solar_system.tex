...
\subsection{Historical comet identification}
\label{sec:historical-comet-identification}

The Historical Comet Identification Mode compares one fitted comet
apparition with an independently preserved sky description. The calculation
does not propagate the present orbit through an ancient close encounter.
CODES selects the NASA/JPL solution for the relevant apparition and records
the numeric solution identifier in every output product. The finder chart
then places the topocentric comet position among Hipparcos stars for the
specified terrestrial site.

The first curated case uses the 1759 \mbox{1P/Halley} record from the
\textit{Seongbyeon Cheukhu Danja}. The manuscript is held by Yonsei
University Library and the digital image was provided by the Korea Astronomy
and Space Science Institute \cite{KASISeongbyeon}. The record describes the
comet within the Xu lunar mansion and north of the Liyu asterism on lunar
March 11. The converted civil date is 1759 April 7 near dawn
\cite{Lee2014Halley}. CODES adopts 20:30 UTC on the preceding civil evening
as the reference epoch for the fifth night watch. The adopted epoch is a
modeled center because the historical time expression has finite width.

CODES selects Horizons apparition record 90000027. At the reference epoch
the calculated ICRS position is
$\alpha=329.7288^\circ$ and $\delta=-9.6505^\circ$. The elevation from
Hanyang is $17.90^\circ$ and the solar elongation is $51.67^\circ$. The
calculated point lies inside the Xu right ascension interval with a
$1.72^\circ$ margin. The point is $4.75^\circ$ north of the adopted Liyu
triplet. The two independent positional statements in the manuscript are
therefore reproduced.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{appendixC_assets/seongbyeon_cheukhu_danja_1759.jpg}
\caption{\textit{Seongbyeon Cheukhu Danja} manuscript for the 1759 Halley
apparition. Yonsei University Library holds the manuscript. The digital image
is provided by the Korea Astronomy and Space Science Institute
\cite{KASISeongbyeon}. Source credit is included here. Redistribution in the
submitted proposal requires confirmation from the image holder.}
\label{fig:seongbyeon-halley-manuscript}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{appendixC_assets/joseon_halley_1759_finder_chart.png}
\caption{CODES reconstruction of the 1759 Halley apparition from Hanyang.
The green band marks the Xu right ascension interval. The cyan triplet marks
Liyu and the gold symbol marks the lunar March 11 reference epoch. The orange
curve uses the NASA/JPL historical apparition solution 90000027.}
\label{fig:joseon-halley-identification}
\end{figure}

\begin{warningbox}
\textbf{\color{cOrange}The polar distance is not yet a validation point.}
The manuscript description reports a north polar distance of
$116^\circ$. The historical JPL solution gives $100.792^\circ$ when the
declination is referred to the apparent equator of date. CODES preserves the
raw value and the $-15.208^\circ$ residual in the provenance JSON but excludes
the value from the identification score. A defensible numerical comparison
requires a checked transcription of the complete page and calibration of the
instrument scale and coordinate convention. The two categorical agreements
demonstrate consistency. They do not by themselves prove the orbit.
\end{warningbox}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{appendixC_assets/codes_historical_gui.png}
\caption{Historical identification controls in the CODES desktop interface.
The user may select a bundled record or enter a comet, UTC epoch, numeric JPL
apparition record, observing site, track duration, sampling, and finder chart
radius. The calculation writes coordinates, the selected solution number,
the record comparison, and complete provenance.}
\label{fig:codes-historical-gui}
\end{figure}
...
virtual-asteroid ensemble. The full multi-body 1PN switch controls both
nominal and ensemble propagation. The other tabs expose comet apparition
histories, apparent sky positions, historical record comparison, and the
validation suites. The run log records the executed command and returned
file paths.}
\label{fig:codes-gui}
\end{figure}

\noindent\begin{tabularx}{\textwidth}{@{}P{31mm}P{58mm}Y@{}}
\toprule
...
\cmidrule(lr){1-3}
\textbf{Historical identification} &
Accepts a curated record or a manually specified comet, epoch, apparition
solution, terrestrial site, and chart scale. &
Topocentric coordinate table, Hipparcos finder chart, historical constraint
comparison, and provenance JSON\\
\cmidrule(lr){1-3}
\textbf{Validation} &
...
\bibitem{Lee2014Halley}
Lee, K. W., Mihn, B. H., \& Ahn, Y. S. 2014,
``Korean Historical Records on Halley's Comet Revisited,''
\textit{Journal of Astronomy and Space Sciences}, 31, 215.
\href{https://doi.org/10.5140/JASS.2014.31.3.215}
{doi:10.5140/JASS.2014.31.3.215}.

\bibitem{KASISeongbyeon}
Korea Astronomy and Space Science Institute 2023,
``Joseon Dynasty comet observation records proposed for the UNESCO Memory
of the World Register.''
\url{https://www.kasi.re.kr/kor/publication/post/newsMaterial/29446}.
