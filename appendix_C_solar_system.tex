...
\caption{The optical size degeneracy from Equation~\ref{eq:h-diameter}.
The horizontal axis is absolute magnitude $H$, the vertical axis is visible
geometric albedo $p_V$, and the color scale at right gives diameter $D$.
White contours mark representative diameters. Thus, the same optical
brightness can imply substantially different sizes as albedo changes.}
...
leaves the diameter uncertain by $p_V^{-1/2}$. However, the thermal flux in the mid-IR has the schematic dependence according to the
Near-Earth Asteroid Thermal Model as
\begin{equation}
 F_{\nu}^{\rm th}
 \mathrel{\propto}
...
The dimensionless quantity $\epsilon$ is the bolometric thermal emissivity:
it multiplies the greybody radiance and also enters the radiative-equilibrium
